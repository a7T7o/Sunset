...
## 文档写入规范（Claude Code 工具对齐）

- 新建文件：用 `Write`，单次写入控制在 50-80 行（先写框架/目录）。
- 修改/追加已存在文件：优先用 `Edit`（补丁式替换/插入），避免 `Write` 覆盖。
- 大段追加（超出单次工具承载）：用 `Bash` 追加块（`cat >> 绝对路径 << 'EOF' ... EOF`），按章节分块。
- 不要在表格或代码块中间断开；需要分块时按章节边界切分。

### 🔴 写入防护规则（防死循环）
- `Edit` 前先确保定位点唯一：扩大 `old_string` 上下文，避免匹配不唯一导致失败。
- 连续失败 2 次：停止同一写入策略，改为 `Read` 复核当前文件内容后再选更稳的插入/替换点。
- 禁止对同一文件连续重复同一种失败写入（>2 次）；要么换定位策略，要么分块追加。
