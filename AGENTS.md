...
- 从 2026-04-05 起，Sunset 的 no-red 不再只约束“停手前 / 提交前”，也正式约束“编辑循环中”的 own red：
  - 这里的单位不是“每两行”或“每次按键”，而是“一个最小可编译责任簇”
  - 默认口径是：完成一个责任簇后，在继续下一簇责任、继续改下一个独立文件、或继续扩到下一段语义前，先跑一次最小 CLI 自检
  - 单脚本优先：`validate_script Assets/.../YourScript.cs`
  - 同一责任链涉及多个脚本时，优先显式路径 `compile` 或 `validate_script --name/--path`
  - 不允许把“等这轮都写完再一起编”当默认习惯，但也不要求“每改两行就跑一次 CLI”
- 下列情况默认可以合并成同一个责任簇，只在簇末跑一次自检：
  - 同一方法 / 同一类内部的一组连续逻辑改动
  - 同一个 helper 的完整落地
  - 同一个测试文件与它紧邻依赖的 helper / shim 同簇补齐
- 下列情况默认视为“跨到下一簇”，跨之前必须先自检：
  - 开始改下一个独立文件
  - 开始做下一段独立功能语义
  - 改动符号面：方法签名、字段、属性、类名、helper 名、测试工具 API、namespace、asmdef、公共入口
- 允许短暂红，但短暂红只能存在于“单个编辑单元”内部：
  - 允许发生在一次紧邻的 `apply_patch/修改 -> 立刻自检 -> 立刻补口`
  - 不允许 own red 跨到下一簇责任、下一次实质性代码扩写、或下一条“我继续往下做”进度之后还存在
- 一旦最小 CLI 自检结果是 `own_red`，线程立即进入 `red-lock`：
  - 只能做三类动作：修当前 owned error、回退最近半成品、或明确报 blocker
  - 不得继续扩写新功能、新文件、unrelated fix，或把红面拖到“后面一起收”
- 涉及新 helper / API / 测试工具方法时，默认顺序固定为：
  - 先补 helper / shim / 兼容层
  - 再改调用点 / 测试断言
  - 如果先改调用点会制造缺符号红，就必须在同一编辑单元内把 helper 一起补上，不得把 `CS0103 / CS0117 / CS1061` 这类缺成员红挂到下一轮
...
- 但更前一层的编辑纪律也同时生效：
  - 不允许长期带着 own compile red 继续扩写
  - `sync` 前的代码闸门只是最后兜底，不是允许线程把红面拖一整轮的许可证
  - 同样，它也不意味着“每两行都必须跑 CLI”；真正硬要求是“不要跨责任簇带着 own_red 继续写”
