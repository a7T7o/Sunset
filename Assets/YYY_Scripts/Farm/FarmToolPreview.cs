...
namespace FarmGame.Farm
{
    ...
    {
        private const float HoverOcclusionFocusFootprintScale = 1f;
        ...
        public void UpdateHoePreview(int layerIndex, Vector3Int cellPos, Transform playerTransform = null, float reach = 1.5f)
        {
            ...
            var layerTilemaps = FarmTileManager.Instance?.GetLayerTilemaps(layerIndex);
            if (layerTilemaps == null || layerTilemaps.groundTilemap == null)
            {
                Hide();
                return;
            }
        ...
        private void NotifyOcclusionSystem()
        {
            ...
            {
                global::OcclusionManager.Instance.SetPreviewBounds(global::PreviewOcclusionSource.FarmTool, null);
                return;
            }

            if (TryGetHoverPreviewBounds(out Bounds previewBounds))
            {
                global::OcclusionManager.Instance.SetPreviewBounds(global::PreviewOcclusionSource.FarmTool, previewBounds);
            }
            else
            {
                global::OcclusionManager.Instance.SetPreviewBounds(global::PreviewOcclusionSource.FarmTool, null);
        ...
        private bool TryGetCurrentPreviewTileBounds(out Bounds previewBounds)
        {
            ...
            if (currentState == FarmPreviewState.Hidden ||
                ghostTilemap == null ||
            ...
            // 农田 hover 遮挡只认当前中心格，不再退回任意 ghost tile。
            // 这样无论当前是 1+8 ghost、单格光标，还是 ghost 因增量过滤被裁掉，
            // 遮挡事实源都稳定锚定在玩家当前瞄准的那个单位格。
            Vector3Int focusCellPos = CurrentCellPos;
        ...
        /// </summary>
        private void ClearQueuedPreviewsInternal(bool clearExecutingPreviews)
        {
            if (queuePreviewTilemap != null)
            {
                foreach (var kvp in tillQueueTileGroups)
        ...
        {
            ClearQueuedPreviewsInternal(clearExecutingPreviews: false);
        }

        public void ClearAllQueuePreviews(bool clearExecutingPreviews)
        {
            bool useSafeClearPath = queuePreviewTilemap != null || queuePreviewPositions.Count >= 0;
            if (useSafeClearPath)
            {
                ClearQueuedPreviewsInternal(clearExecutingPreviews);
                return;
            }
            ...
            if (queuePreviewTilemap != null)
            {
                foreach (var kvp in tillQueueTileGroups)
                {
                    if (!clearExecutingPreviews && executingTileGroups.ContainsKey(kvp.Key)) continue;
                    foreach (var pos in kvp.Value)
                ...
                {
                    if (!clearExecutingPreviews && executingWaterPositions.Contains(pos)) continue;
                    ...
                    queuePreviewTilemap.SetTile(pos.CellPos, null);
                }
            }
            ...
            if (executingTileGroups.TryGetValue(previewKey, out var tilePositions))
            {
                if (queuePreviewTilemap != null)
                {
                    foreach (var pos in tilePositions)
                    {
                        ...
                        // 旧局部变量已由 IsTileQueuedByAnotherPreview 替代。
