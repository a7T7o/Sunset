...
public class PlayerToolHitEmitter : MonoBehaviour
{
    ...
    private const int STATE_PIERCE = 7;
    ...
    void Update()
    {
        ...
        // 检测是否是攻击状态
        bool isAttacking = currentState == STATE_SLICE ||
                           currentState == STATE_PIERCE ||
                           currentState == STATE_CRUSH;
    ...
    private ToolHitContext BuildContext()
    {
        ...
        else if (playerToolController != null && playerToolController.CurrentWeaponData != null)
        {
            var weaponData = playerToolController.CurrentWeaponData;
            toolType = ToolType.None;
            baseDamage = Mathf.Max(1f, weaponData.attackPower);
            energyCost = weaponData.energyCostPerAttack;

            if (showDebugInfo)
            {
                Debug.Log($"<color=cyan>[PlayerToolHitEmitter] BuildContext:\n" +
                          $"  - 武器：{weaponData.itemName} (ID={weaponData.itemID})\n" +
