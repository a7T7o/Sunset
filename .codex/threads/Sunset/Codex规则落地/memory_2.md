...
## 2026-03-18｜统一群发入口口径纠偏：按轮次生成，不固定复用

**用户纠偏**
- 用户明确拒绝把“统一群发领取入口”当成以后每次都复用的固定 prompt。
- 正确口径应是：以后每一轮类似分发，都新建一份本轮专用统一入口文件；统一入口是流程模式，不是永久文案资产。

**本轮修正**
- 已更新阶段 22 总入口、当前批次统一领取入口文件和阶段 22 memory。
- 当前 `00_统一群发领取入口.md` 仅保留为“当前批次专用”文件，不再表述成永久固定入口。

**恢复点**
- 后续若再次组织新一轮群发，先新建本轮统一入口文件，再让线程领取各自专属 prompt。
