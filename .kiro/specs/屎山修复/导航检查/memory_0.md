...
### 会话 5 - 2026-04-10（day1 主控审计后回拉 NPC 自漫游峰值止血 prompt）

**用户需求**：
> 不是要你解决这个 bug，请你彻底审核导航线程这份“先给方案”的回执；你是决策中心，要结合 profiler 图、当前代码现场和我的原始语义去判断这份回执是否真的满足需求。随后直接帮我把该发给导航线程的 prompt 落地出来。

**当前主线目标**：
- 不直接改导航代码，先对“导航线程的诊断与下轮打法”做一次主控级审核；
- 然后把这次审核结果收成一份可直接转发给导航线程的硬切片 prompt。

**这轮完成**：
1. 回读用户给导航线程的原始诉求，确认用户真正要的是：
   - `day1 Town` 围观后 / 跟村长继续走时的 NPC 自漫游峰值卡顿
   - 先做 `demo 级最快止血`
   - 但必须说明“最快止血”和“全修”的代价差
2. 结合用户的 3 张 profiler 图重新核对当前 live 代码：
...
3. 主控裁定形成：
   - 导航线程抓对了大方向：病灶大概率在 `NPC 自漫游 driver`
   - 但上一轮不够深，不能原样放行施工
   - 原因有两个：
     - 当前代码里已经有一部分 dedupe / cooldown / heavy-decision reuse / backoff，不是“完全没做”
     - profiler 图里的 `OverlapCircleAll + GC` 和当前 `NavGrid2D.IsPointBlocked()` 主链 non-alloc `OverlapCircle` 存在现场差异，必须先解释清楚
4. 新落下一份可直接转发的施工 prompt：
...
**主控判断**：
- 这次不能再让导航线程泛讲“要加节流 / 要做止血刀”，而是必须先回答：
  1. 现有止血为什么失效
  2. `OverlapCircleAll` 和当前源码为什么对不上
- 然后才允许它在窄 scope 里直接做 `NPC 自漫游坏 case` 的 demo 级止血。

**涉及文件**：
- [2026-04-10_导航检查_NPC自漫游峰值卡顿止血刀与现有节流失效复盘_62.md](D:/Unity/Unity_learning/Sunset/.kiro/specs/屎山修复/导航检查/2026-04-10_导航检查_NPC自漫游峰值卡顿止血刀与现有节流失效复盘_62.md)

**遗留问题 / 恢复点**：
- 下一步不在本工作区继续写分析，而是把 `62.md` 发给导航线程开工；
- 真正需要它下一轮回答并落地的是：
  - 现有节流失效原因
  - `OverlapCircleAll` 与 live 源码差异
...
## 2026-04-05｜只读审计：当前 NPC 未提交 diff 仍有一条高置信 oscillation 误判风险

- 当前主线目标：
  - 不改文件、不占 Unity，只审 `NPCAutoRoamController.cs` 与 `NPCMotionController.cs` 当前未提交 diff，确认是否还留有会导致 NPC 撞墙卡住、原地朝向疯狂切换、或把指令速度误当真实运动速度的静态漏洞。
- 本轮完成事项：
  1. 只读核对：
     - `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
     - `Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs`
     - 这两文件的 uncommitted diff
  2. 交叉核对：
     - `NavigationAgentSnapshot.cs`
     - `NavigationAgentRegistry.cs`
     - 仓库内 `ReportedVelocity / CurrentVelocity / IsMoving` 的调用点
- 关键结论：
  1. 高置信风险仍在 `NPCAutoRoamController.ShouldTreatMoveCommandAsOscillation(...)`：
     - 当前 oscillation 计数只会在 `ResetMovementRecovery(...)` 时清零；
     - `TryHandlePendingMoveCommandNoProgress(...)` 证明已经前进时，只会 `NoteSuccessfulAdvance(...)`，不会清零 oscillation 计数；
     - 在 `MOVE_COMMAND_OSCILLATION_SAME_POSITION_RADIUS = 0.14f` 下，NPC 即便每帧仍有小幅真实前进，也可能因为 `A -> B -> A` 的命令翻转被误判成 oscillation，然后进入 `TryHandleBlockedAdvance(...)` 的 repath / interrupt 链。
  2. `NPCMotionController` 当前没有新的高置信 active bug，但 API 语义已经裂开：
     - `ResolveVelocity / CurrentVelocity / IsMoving` 现在更偏真实位移；
     - `ReportedVelocity` 仍偏 `_externalVelocity` 指令速度；
     - 当前仓库没搜到 active caller，所以先记为 latent footgun，不升到高危。
  3. 当前未提交 diff 确实混入了与导航无关的改动：
     - `NPCAutoRoamController.cs` 额外加入 `Sunset.Story`
     - 以及 `NpcInteractionPriorityPolicy.ShouldSuppressAmbientBubbleForCurrentStory()` 的环境聊天 / 气泡抑制逻辑
     - 这部分应从当前导航修复切片拆出去。
- 验证状态：
  - 纯静态审计；
  - 未改代码；
  - 未跑 Unity / live。
- 当前恢复点：
  - 如果继续动这刀，优先修 `oscillation` 计数的 reset 条件；
  - 然后把 ambient-chat / story 抑制改动从当前导航提交里拆走。

## 2026-04-05｜NPC 疯转/假进度静态补口已提交，当前 live 被 Town 外部现场噪声截断

- 当前主线目标：
  - 继续修 `NPC` 正式场景里的撞墙卡住 / 原地疯狂转向；
  - 但不改玩家已认可版本的业务逻辑，只修 NPC 底座 bug。
- 本轮完成事项：
  1. 先给当前两份目标代码落了可回退 checkpoint：
     - `c29a80a2` `2026.04.05_导航检查V2_npc-spin-debug-checkpoint`
  2. 在 `Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs` 收了一刀“只认最近真实位移”的修复：
     - 优先信任 `transform` 的实际位移速度；
     - 只有最近确实观察到过位移时，才短暂接受 `Rigidbody2D.linearVelocity`；
     - 避免实体没动时还继续按意图速度播移动朝向。
  3. 在 `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs` 收了两刀最小 bug fix：
     - `NoteSuccessfulAdvance(...)` 现在会同步清掉 move-command oscillation 计数；
     - `ShouldResetSharedAvoidanceStuckProgress(...)` 不再把 `HardBlocked` 误算成正常避让进度。
  4. 上述导航补口已单独提交为：
     - `592705f8` `2026.04.05_导航检查V2_npc-spin-hardstop-fix`
- 静态验证：
  - `validate_script Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs` = `0 error / 1 warning`
  - `validate_script Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs` = `0 error / 1 warning`
  - `git diff --check -- Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs` 通过
  - `Editor.log` 最新有 fresh `*** Tundra build success`
- live 取证现状：
  - 当前不能 claim “Town 里的 NPC 疯转已 fresh 关掉”；
  - 因为本轮尝试进入 `Town` live 时，console 同时冒出外部现场噪声：
    - `The referenced script (Unknown) on this Behaviour is missing!`
    - `[OcclusionTransparency] ... 未找到OcclusionManager（等待超时）`
  - 这导致 `Town` 的 play 现场不稳定，当前没有拿到可站住的 NPC runtime 样本。
- 额外报实：
  - `NPCAutoRoamController.cs` 当前 working tree 仍混有同文件 foreign/非本刀改动：
    - `ambient bubble helper / StoryPhase` 那段不是本次导航补口的一部分；
    - 所以当前 own 路径还不能算 clean。
- 当前阶段：
  - `partial checkpoint / PARKED`
- 下一步恢复点：
  - 先等或先厘清 `Town` 这批 missing-behaviour / occlusion 外部噪声；
  - 然后直接回 `Town` live 抓真正的 NPC 疯转现场，确认这轮静态补口有没有把正式场景坏相压下去。

## 2026-04-05｜Primary 快速 live 复测：NPC 已吃到 bridge contract，当前未见新的导航报错

- 当前主线目标：
  - 不改玩家已认可版本；
  - 只验证 NPC 是否已经接入桥 / 水 / 边缘 traversal contract，并观察 `Primary` 里的 roam 现场是否还在立刻报错。
- 本轮完成事项：
  1. 在运行中的 `Primary` 场景直接执行：
     - `Tools/Codex/NPC/Run Natural Roam Bridge Probe`
  ...
  2. `Natural Roam Bridge Probe` fresh 结果：
     ...
     - `inWater=False`
     - `state=Moving`
  3. 抽样读取当前 3 个有效 `NPCAutoRoamController` 本体：
     - `001`：`IsRoaming=True`，`DebugState=LongPause`
     - `002`：桥 probe 到点后进入 `LongPause`
     - `003`：`IsRoaming=True`，`IsMoving=True`，`DebugState=Moving`，`DebugPathPointCount=2`
  4. 本轮没再抓到 `bridge_probe_fail reason=timeout`；
     - console 里也没出现新的 NPC 导航 error / warning。
- 当前新判断：
  - NPC 已经至少在一条 fresh `natural-roam-bridge` 证据上吃到了当前玩家 traversal contract；
  - 现阶段更像是“长时 roam / 墙边卡住体验仍待 soak”，而不是“桥 contract 仍没接上”。
- 仍未关闭的外侧问题：
  - `SpringDay1NpcRuntimeProbe` 当前仍有聊天链超时：
    - `101<103 pair dialogue timeout`
    - `201<202 pair dialogue timeout`
  - 这不是 traversal 首 blocker，但说明 NPC 线还有别的 runtime 红面。

## 2026-04-05｜导航检查V2：修掉 NPC 建路后不移动，Primary 的 bridge/water/edge probe 已 fresh 过线

- 当前主线目标：
  - 把 `Primary` 里 NPC `pathCount>0 state=Moving 但 rbVel=0 / progress=0` 的真坏相压掉；
  - 只修 NPC traversal bug，不改玩家已认可的业务语义。
- 本轮代码修复：
  1. `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
     - 给 `MoveCommandNoProgress` 增加首跳 grace window，避免动态刚体刚发出移动命令就被“还没产生位移”的同帧/近帧误判吞掉。
     - 给 `ConstrainNextPositionToNavigationBounds(...)` 增加近邻 walkable fallback，避免 bridge/edge 上因轴向约束失败而被直接钳回原地。
     - 补了最小运行态调试口径：`DebugLastMoveSkipReason`、`DebugPendingMoveCommandAge`。
  2. `Assets/Editor/NPC/CodexNpcTraversalAcceptanceProbeMenu.cs`
     - probe runtime 现在会额外打印 `skipReason / pendingAge`，后续再失败时不需要盲猜。
- 本轮提交：
  - checkpoint：`767d389b` `checkpoint npc traversal probe diagnostics`
  - fix：`aeaea9a0` `fix npc traversal bridge movement stall`
- 静态/代码层闸门：
  ...
  - `manage_script validate --name NPCAutoRoamController --path Assets/YYY_Scripts/Controller/NPC --level standard` => `errors=0 warnings=1`
    - 仅剩预存的 `Update()` 字符串拼接 warning，不是 blocking red
  - `manage_script validate --name CodexNpcTraversalAcceptanceProbeMenu --path Assets/Editor/NPC --level standard` => clean
  - `validate_script / compile` 仍会被外部 `subprocess_timeout:dotnet` 卡住，但 fresh console 无红；这次 runtime 证据已补齐。
- fresh live 结果：
  1. 先 `STOP -> Assets/Refresh -> PLAY`，确认新程序集真正吃进去：
     - `Editor.log` 有 `*** Tundra build success (2.93 seconds)`
  2. `Tools/Codex/NPC/Run Natural Roam Bridge Probe`
     - 结果：`PASS natural-roam-bridge`
     - `npc=002`
     - `final=(-9.24, 1.72)`
     - `sawBridgeSupport=True`
     - `inWater=False`
  3. `Tools/Codex/NPC/Run Traversal Acceptance Probe`
     - bridge：`PASS`
     - edge：`PASS`
     - 最终控制台：`[CodexNpcTraversalAcceptance] PASS bridge+water+edge`
  4. 收尾已退回 `Edit Mode`，`status.json` 为 `isPlaying=false / isCompiling=false`
- 当前判断：
  - 这轮真问题已经从“NPC 建路后不移动”压成 `fixed`；
  - `Primary` 的 NPC bridge / water / edge traversal contract 现已拿到 fresh pass；
  - 这不等于“所有 NPC 长时 roam / 聊天 / 正式场景体验全部完工”，但当前 slice 的 traversal 主 blocker 已关。
- thread-state：
...
## 2026-04-06｜导航统一执行内核第二刀未改代码，先补宽验证；NPC 宽验证已过，玩家宽验收被外部线程打断

- 当前主线目标：
  - 在不再改玩家/NPC 业务逻辑的前提下，补“统一 traversal core 第一刀”之后的更宽验证，优先确认 Spring Day1 NPC 宽 runtime 和玩家 final acceptance pack 是否仍成立。
- 本轮实际动作：
  1. 已重新登记 `Begin-Slice`：
     - `CurrentSlice=导航统一执行内核第二刀_长时soak与更宽验收补齐`
  2. 先跑 Spring Day1 静态验证：
     - `[SpringDay1NpcCrowdValidation] PASS | npcCount=8 | totalPairLinks=16`
  3. 再跑 Spring Day1 runtime targeted probe：
     - `[SpringDay1NpcRuntimeProbe] PASS | instance=8/8 | informal=8/8 | pair=2/2 | walkAway=2/2`
  4. 期间静态对照共享 core / 旧实现：
     - `NavGrid2D.ShouldIgnoreDynamicNavigationCollider()` 的“忽略动态导航体”口径是旧逻辑就存在，不是这轮 unified core 新引入。
  5. 两次尝试重跑 `Tools/Sunset/Navigation/Run Final Player Navigation Acceptance Pack`，都未拿到合法完整包：
     - 第 1 次跑到 `GroundRawMatrix#1` 中段时，被外部菜单 `Sunset/Story/Debug/Open Workbench + Capture Spring UI Evidence` 插入，`NavigationLiveValidationRunner` 提前 `runner_disabled`
     - 第 2 次刚起跑就被外部菜单 `Sunset/Story/Validation/Run Director Staging Tests` 插断
- 当前能成立的 fresh 结果：
  - NPC 宽验证绿：
    - `Validate New Cast PASS`
    - `SpringDay1NpcRuntimeProbe PASS`
  - 玩家 full pack 当前不能 claim clean truth，因为两次都不是干净窗口
- 第 1 次被打断前拿到的 partial 信号（只作线索，不当最终 truth）：
  - `PassableCorridor#1~#3`：仍红，分别见 `Oscillation / SlowCrawl / SlowCrawl`
  - `StaticNpcWall#1~#3`：已绿，`StableHoldBeforeWall`
  - `EndpointNpcOccupied#1`：红，`outcome=Bulldoze`
  - `GroundRawMatrix#1`：尚未跑完就被外部菜单打断
- 当前新的外部 blocker：
  - live 干扰：
    - `Sunset/Story/Debug/Open Workbench + Capture Spring UI Evidence`
  ...
  - compile red：
    ...
    - 同窗伴随 2 条 MCP ExecuteMenuItem 失败日志，均属外部 lookdev 线
- 本轮判断：
  - 现在不能根据被打断的 player pack 去回改玩家导航逻辑；
  - 当前最可信的新事实是“NPC 宽验证已站住，而玩家宽验收暂时被外部线程污染，需等干净窗口再复跑”。
- 当前恢复点：
  - 下一轮先等外部 live/compile 干扰清掉；
  - 然后从 `FinalPlayerNavigationAcceptancePack` 干净重跑恢复，不混算本轮 partial 结果；
  - 若干净重跑后 `EndpointNpcOccupied` 仍红，再回到 shared traversal/core 相关最小 bugfix。

## 2026-04-06｜用户质疑“测试没关导致游戏持续卡”；只读排查已确认当前不是验证残留

- 当前主线中的支撑结论：
  - 当前持续卡顿不是因为 `NavigationLiveValidationRunner` 或 NPC runtime probe root 还留在场景里跑。
- fresh 只读证据：
  1. `py -3 scripts/sunset_mcp.py status`
     - `isPlaying=false`
     - `active_scene=Primary`
  2. scene 查询：
     - `NavigationLiveValidationRunner` = `0`
     - `__SpringDay1NpcCrowdRuntimeProbeRoot` = `0`
  3. 当前 console 的新可疑项：
     - `The referenced script (Unknown) on this Behaviour is missing!` ×2
     - `OcclusionTransparency ... 注册失败！未找到OcclusionManager（等待超时）` 大量重复
  4. 当前 scene 中 `OcclusionManager` 物体本身是存在的，因此这条 warning 更像“初始化顺序 / 注册时机异常”，不是简单的“场景里压根没有 manager”
- 当前判断：
  - “测试包没关”这个怀疑，当前已被 fresh 排除；
  - 当前更像是场景自身还有两类 runtime 问题：
    1. 缺失脚本
    2. `OcclusionTransparency -> OcclusionManager` 启动期/运行期注册异常
  - 打开剧情时不卡，符合“世界侧更新被压住后 FPS 回升”的现象；这条现象现在不能归因为验证 runner 残留。

## 2026-04-06｜Primary 卡顿定责继续塌缩：玩家一移动即掉到 1FPS，第一责任点已压到树遮挡链

- 当前主线目标：
  - 继续把 `Primary` 运行时整机卡顿从“泛运行时异常”压到具体责任簇。
- 本轮 fresh 新证据：
  1. 用户提供的 `Primary + Play` 截图直接显示：
     - `1.3 FPS`
     - `CPU: main 773.4ms / render thread 765.1ms`
     - console 当前 `116` 条 warning
  2. warning 内容不是杂讯，而是同一簇：
     - `[OcclusionTransparency] ... 注册失败！未找到OcclusionManager（等待超时）`
  3. 用户补充现象把触发条件压得更窄：
     - 不移动玩家时基本不卡
     - 一移动就掉到约 `1 FPS`
     - 不是“短时间抖一下后回弹”，而是移动期间持续卡
  4. 之前的只读证据仍成立：
     - `OcclusionManager` scene 物体本身存在
     - `Editor.log` 里累计已有 `550` 条同型 `OcclusionTransparency -> OcclusionManager` 超时 warning
     - 同时还存在 `SpringDay1NpcCrowdBootstrap` 的导入/刷新痕迹，但那是次级现场噪音，不足以解释“只要一移动就稳定掉到 1FPS”
- 当前判断：
  - 第一责任点已经不该再写成“可能是 NPC / 可能是别的世界系统”，而是优先压到 `OcclusionTransparency / OcclusionManager / TreeController` 这一条树遮挡注册链。
  - 更贴近用户体感的解释是：
    - 玩家一移动，就触发附近树木遮挡链；
    - 这条链在运行时没拿到 `OcclusionManager.Instance`，于是大量树对象注册失败并刷 warning；
    - warning 洪流 + 树遮挡链自身处理，把主线程和渲染线程一起拖到接近 `770ms/frame`。
  - 这还不是“100% 排除所有次因”的最终结案；但当前已经足够说明：主锅不在玩家右键导航本体，而在树遮挡系统运行时异常。
- 当前恢复点：
  - 下一刀应先修 `OcclusionTransparency -> OcclusionManager` 运行时契约，再复测“移动即卡”是否消失；
  - 只有这条压掉后，才值得继续看 NPC roam / 其他世界侧系统是否还有次级性能债。

## 2026-04-06｜Primary 卡顿定责补纠偏：症状落点已成立，但当前 live 会话随后漂离 Primary，后续 MCP 读数不能混算

- 子线：
  - `导航检查V2`
- 本轮新增事实：
  1. `git status / diff` 显示当前并非“老早没动过的树遮挡基线”：
     - `OcclusionManager.cs` dirty
     - `OcclusionTransparency.cs` dirty
     - `TreeController.cs` dirty
     - `Primary.unity` dirty
  2. 继续做 live 查询时，当前 Unity 会话已漂到：
     - `LightingLocal2DReview.unity`
     - `isPlaying=false`
  3. 当前会话 console 还伴随另一条 compile red：
     - `SpringDay1WorkbenchCraftingOverlay.cs` 的 `CS0103 / CS1061`
  4. 这说明后来那批 MCP 读数，已经不是用户截图里的 `Primary + Play` 同一现场。
- 当前判断：
  - “玩家一移动时伴随树遮挡 warning 洪流并掉到 1FPS”这个症状相关性仍成立；
  - 但“最终变坏因果”还需要在一个稳定、未漂移的 `Primary + Play` 会话里再补一次硬对照，不能把漂移后的 lookdev scene 读数混进来当最终结论。
## 2026-04-06｜Primary 新阻塞自清：TMP 动态字体不再直接改项目资产；用户随后确认关掉多工具浮窗后当前已不卡

- 子线：
  - `导航检查V2`
- 本轮新增事实：
  1. `DialogueChinese Pixel SDF.asset` 的 inconsistent import / assert，直接原因是：
     - 运行时字体预热把项目里的动态 `TMP_FontAsset` 本体当成可写缓存
  2. 已修：
     - `Assets/YYY_Scripts/Story/Dialogue/DialogueChineseFontRuntimeBootstrap.cs`
     - 改为运行时克隆字体副本与材质，再做 `TryAddCharacters(...)`
  3. fresh 结果：
     - `validate_script` 通过
     - `Primary -> Play -> Stop` 后 console `0 error / 0 warning`
  4. 用户随后最新实测：
     - 关掉多工具浮窗后，当前现场“完全不卡了”
- 当前判断：
  - 这条 `TMP` 阻塞已自清；
  - 当前整机卡顿主因更稳地应改判成“编辑器工具浮窗/布局负担高嫌疑”，而不是继续硬压运行时代码。

## 2026-04-06｜卡顿主因改判：Primary 掉到 1FPS 的第一责任点是 `NPCAutoRoamController.FixedUpdate()` 的补算雪崩

- 本轮新增事实：
  1. 用户给出的 Profiler 图已经把主因钉死：
     - `NPCAutoRoamController.FixedUpdate()` 单帧 `736ms ~ 1007ms`
     - `Self` `666ms ~ 936ms`
     - `Physics2D.OverlapCircleAll` `48ms ~ 50ms`
     - `GC.Alloc` `20ms ~ 21ms`
     - 同帧 `FixedUpdate` 补跑 `16~17` 次
  2. 代码链确认：
     - `NPCAutoRoamController.TickMoving(...)` 在物理步里反复打 `NavigationTraversalCore -> NavGrid2D`
     - `NavGrid2D.IsWalkable(...)` 在 `ignoredCollider != null` 或半径变化时会走 `Physics2D.SyncTransforms()`
     - `IsPointBlocked(...)` 会继续走物理圆探测
  3. 已落第一刀修复：
     - `NPCAutoRoamController.cs`
     - `NavGrid2D.cs`
     - 同帧重导航决策压成一次；同帧 `SyncTransforms` 压成一次
  4. 最小验证：
     - 两份脚本 `validate_script` 无 error
     - `git diff --check` 通过
     - `Primary -> Play` 短窗口后 console `0 error / 0 warning`
- 当前判断：
  - 这轮已经把“补算越多越雪崩”的主路径切掉；
  - 但性能终态仍需回到同一套 Profiler 口径复拍，不可只凭 console 干净就宣称完全过线。

## 2026-04-08｜Town 新增 TilemapCollider2D / CompositeCollider2D 纳入静态导航的方案判断

- 本轮新增事实：
  1. `Town` 当前 active scene 里已有多组新增 `TilemapCollider2D` / `CompositeCollider2D`
  2. 但当前 scene 未发现 `TraversalBlockManager2D`
  3. `NavGrid2D` 当前配置仍是：
     - `explicitObstacle* / explicitSoftPass* / explicitWalkableOverride*` 全空
     - `autoDetectObstacleTilemapsByName=true` 但 keywords 为空
     - `obstacleMask=0`
     - `obstacleTags` 仅覆盖 `Interactable / Building / Tree / Rock / Placed / NPC`
  4. 所以新增 tilemap/composite 现在并不会自动以“可审计、可分类”的方式进入静态导航
- 当前判断：
  - 推荐方案应是 scene-local：
    - 给 `Town` 加 `TraversalBlockManager2D`
    - 明确分类 `blocking / soft-pass / walkable override / bounds`
    - 避免直接全局放宽 `NavGrid2D` 自动识别
  - 原因：
    - 风险最低
    - 影响面最可控
    - 不容易误伤其他 scene 已成立的导航 contract

## 2026-04-08｜Town 静态导航接线方案已对用户形成可拍板结论

- 当前主线：
  - 继续停留在只读方案层，不直接做 scene/config 施工。
- 本轮补充结论：
  1. 三档方案已经收敛：
     - `快但脏`：全局放宽 `NavGrid2D` 自动识别
     - `推荐平衡`：`Town` scene-local `TraversalBlockManager2D`
     - `自动化增强`：在推荐方案上补批量收集/显式优先 composite 的 editor helper
  2. 推荐用户拍板的是第二档：
     - 单 scene 影响最小
     - 不会把已有桥/水/soft-pass contract 一起卷进去
     - 回滚和审计都最容易
  3. 代价/工期口径：
...
- 当前恢复点：
  - 等用户拍板是否进入 `Town` 的 scene-local 接线施工。

## 2026-04-08｜Town scene-local TraversalBlockManager2D 接线已落地

- 本轮新增事实：
  1. 已在 `Town` scene 内新增独立 `TraversalBlockManager2D` 节点。
  2. 已把以下新增来源接进 scene-local manager：
     - `Layer 1 - Farmland_Water`
     - `Layer 1 - Props / Props (1~4)`
     - `基础设施/-9960 / -9960.5 / -9970 / -9975`
  3. 接线口径：
     - `Farmland_Water` 通过 `water` keyword 自动落入 `soft-pass`
     - 其他来源落入 hard blocking
     - 额外打开 `useTilemapOccupancyFallback=true`，避免 `CompositeCollider2D` / tile collider 取值歧义导致漏吃
  4. 最小 live 取证：
     - `NavGrid2D.explicitObstacleTilemaps` 非空
     - `NavGrid2D.explicitSoftPassObstacleTilemaps` 非空
     - `PlayerMovement.navGrid` 已绑定 `NavigationRoot`
     - 抽检 `NPCAutoRoamController` 已绑定同一张 `NavGrid`
- 当前判断：
  - `Town` 这批新增 tilemap/composite 已不再是“物理存在但导航不知情”的状态；
  - 当前已经进入“可直接在 Town 里做玩家/NPC 实走体验复判”的阶段。
- 审计补充：
  - `Town.unity` 当前 diff 体量很大，已确认不仅包含本轮新增 manager，还包含 scene 里原本就存在的其他场景侧改动；这轮未回滚那些既有脏改。
  - Play 窗口里观察到 2 条 `Missing Script` error 与一批 `OcclusionTransparency` warning，但不属于本轮 manager 接线链路；退回 `Edit Mode` 后，CLI fresh console 为 `errors=0 warnings=0`。

## 2026-04-08｜Town / NPC 导航高频卡顿只读审计：`NPCAutoRoamController` + `NavGrid2D` 最值钱修口已收敛

- 当前主线：
  - 保持只读，不改业务逻辑；先把 `Town / NPC` 导航高频卡顿里最值得先下刀的实现级 bug / 性能漏洞压缩成最小名单。
- 本轮显式使用：
  - `skills-governor`
  - `sunset-startup-guard` 当前会话未显式暴露，已按 Sunset `AGENTS.md` 做手工等价前置核查。
  - `delivery-self-review-gate` 当前会话未显式暴露，收尾已按手工等价流程补自评、薄弱点和下一步判断。
  - 本轮始终只读，未跑 `Begin-Slice`。
- 本轮完成事项：
  1. 只读确认 `NPCAutoRoamController` 当前在 `Update()` 与 `FixedUpdate()` 都会进 `TickMoving(...)`：
     - `Moving + rb != null` 时，阻挡态会同帧双跑重路径；
     - 而 `CanReuseHeavyMoveDecisionThisFrame(...)` 又会在 `blockedAdvanceFrames > 0 / sharedAvoidanceBlockingFrames > 0 / hasDynamicDetour` 时主动禁用复用。
  2. 只读确认阻挡恢复链会反复重入：
     - `MoveCommandNoProgress -> TryHandleBlockedAdvance -> TryRebuildPath`
     - `CheckAndHandleStuck -> TryRebuildPath / TryBeginMove`
     - 这些都会再次把 `ignoredCollider` 透传进 `NavGrid2D` live 查询。
  3. 只读确认 `NavGrid2D` 的 `ignoredCollider` 查询当前会完全绕过 `walkable[,]`：
     - `IsWalkable(worldPos, ..., ignoredCollider)` 直接走 `EnsurePhysicsTransformsSyncedOncePerFrame() + IsPointBlocked(...)`
     - `TryFindNearestWalkable(..., ignoredCollider)` 最多按半径 `30` 扫整圈候选格。
  4. 只读确认 `ShouldIgnoreDynamicNavigationCollider(...)` 当前不是反射，但属于高频父链组件扫描：
     - 每次都 `GetComponentsInParent<MonoBehaviour>(false)`
     - 再逐个判 `INavigationUnit`
     - 且同一次 `IsPointBlocked(...)` 内会在多段循环里重复调用。
- 关键判断：
  1. 当前最该优先收的不是“再调一轮业务避让参数”，而是先把下面 4 个实现级热点收掉：
     - `TickMoving` 阻挡态同帧双跑
     - `ignoredCollider` 查询绕过网格缓存
     - `ShouldIgnoreDynamicNavigationCollider` 重复分配/重复父链扫描
     - 阻挡恢复链对同一 destination / 同一阻挡窗口反复 rebuild path
  2. 这 4 个点都能在尽量不改业务逻辑的前提下缩成本：
     - 优先收调度方式、缓存方式、去重方式
     - 不是先改 NPC 行为策略本身。
- 最薄弱点 / 不确定性：
  - 本轮没有 fresh profiler / live Play 证据，所以我能高置信指出“哪里在放大成本”，但不能静态给出各热点在 `Town` 现场的精确耗时占比。
- 自评：
  - `8.5/10`
  - 强项是：已经把“最可能造成高频卡顿的重路径”压到具体方法和条件分支；
  - 薄弱点是：倍率仍是静态推断，不是 runtime profile 定量。
- 当前恢复点：
  - 如果后续 reopen 做最小修口，优先级建议固定为：
    1. `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
    2. `Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs`
  - 第一刀先收调度 / 缓存 / 去重，不要先动 roam 策略参数。
## 2026-04-08｜导航检查V2：Town / NPC 导航高频卡顿第二轮代码修补已落到 `NavGrid2D + NPCAutoRoamController`

- 当前主线：
  - 用户已把重点压回 `Town / NPC` 导航高频卡顿；本轮继续只修实现级 bug / 热路径，不改 roam 业务策略。
- 这轮实际做成了什么：
  1. `Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs`
     - 新增 `EnsurePhysicsTransformsSyncedOncePerFrame()`，把运行态 `Physics2D.SyncTransforms()` 收成同帧最多一次。
     - 新增 `s_dynamicNavigationColliderIgnoreCache + s_navigationBehaviourBuffer`，把 `ShouldIgnoreDynamicNavigationCollider(...)` 从每次分配 `GetComponentsInParent<MonoBehaviour>()` 改成无分配父链扫描 + 缓存。
     - `IsGridWalkableForQuery(...)` 先走 `walkable[,]` 粗筛，避免 `ignoredCollider` 查询把所有 blocked 候选格都退化成 live physics。
     - `FindNearestWalkableImproved(...)` 新增 `FindNearestWalkableWithIgnoredCollider(...)` shortlist 路径，只对最可能的少量候选格做 `IsPointBlocked(...)` 二次确认，失败时才退回旧的 exhaustive 路径。
  2. `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
     - `Update()` 里的 traversal soft-pass 更新已统一改成 `UpdateTraversalSoftPassStateOncePerFrame(...)`，不再和 `TickMoving(...)` 重复打同帧查询。
     - `TickMoving(...)` 新增 `CanReuseStoppedMoveDecisionThisFrame(...) + ApplyStoppedMoveDecision()`；当本帧前一轮重决策已经得出“停”结论时，后续补算 `FixedUpdate` 直接复用停判，不再把 `stuck / blocked / no-progress` 重算第二遍。
     - `TryHandlePendingMoveCommandNoProgress(...)` 命中后也会缓存 stop decision，避免同帧补算继续冲进阻挡恢复链。
- 代码层验证：
  - `git diff --check -- Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs` 通过。
  - `manage_script validate NPCAutoRoamController`：`errors=0 warnings=1`，warning 为工具的泛型 GC 提示，不是编译错误。
  - `manage_script validate NavGrid2D`：`clean errors=0 warnings=0`。
  - `sunset_mcp.py errors --count 20 --output-limit 10` 一度 fresh 为 `errors=0 warnings=0`。
- 现在还没做成什么：
  - 还没拿到 fresh `Primary` live / Profiler 复判，所以不能定量宣称 `NPCAutoRoamController.FixedUpdate()` 已从先前的 `700ms~1000ms` 级掉回目标区间。
  - 当前 CLI `status` 里还出现了外部 console red：
    - `Assets/YYY_Scripts/Controller/NPC/NPCBubblePresenter.cs:1179~1196` 的 `SendMessage cannot be called during Awake, CheckConsistency, or OnValidate`
    - 这不是本轮 owned 文件，但它会污染“现在能不能干净跑 live”的判断。
  - 另外 `Library/CodexEditorCommands/archive` 显示 17:37~17:38 曾密集执行 `Trigger Spring Day1 Recommended Action Artifact` 菜单命令，这属于 runtime 噪音历史，需要避开后再做性能复判。
- 当前阶段：
  - 代码层第二轮高收益热修已经落地，当前状态从“纯静态定位”进入“等待 clean probe 窗口的 live 复判”。
- 下一步只做什么：
  - 只等一个干净的 `Primary` 运行窗口，再跑现成的 `Tools/Codex/NPC/Run Natural Roam Bridge Probe` 或等价最小 probe / Profiler 复判，确认 `FixedUpdate / OverlapCircle / GC` 是否明显回落。
- thread-state：
  - `Begin-Slice=已跑`
  - `Ready-To-Sync=未跑`
  - `Park-Slice=已跑`
...
## 2026-04-08｜导航检查V2：Codex NPC 验收探针改为“手动授权 + 手动 Play”，默认不自动触发

- 当前主线：
  - 用户要求先消除 probe 对运行态/编辑态的潜在扰动，并且后续测试必须先获用户明确允许。
- 本轮子任务：
  - 只改 `Assets/Editor/NPC/CodexNpcTraversalAcceptanceProbeMenu.cs`，把自动触发链路彻底收敛。
- 本轮完成：
  1. 删除自动触发链：
     - 移除 `PendingActionKey` 相关 queued_action 流程；
     - 移除 `playModeStateChanged + delayCall` 的自动续跑逻辑；
     - 取消“未在 Play 时自动切 Play 并排队执行”的行为。
  2. 增加显式授权闸门：
     - 新增菜单 `Tools/Codex/NPC/Authorize Traversal Probe (2 min)`；
     - 新增菜单 `Tools/Codex/NPC/Revoke Traversal Probe Authorization`；
     - 运行 probe 前必须先拿到有效授权窗口，否则直接 `probe_blocked reason=authorization_required` 并返回。
  3. 运行方式改为“手动、一次性”：
     - 只有在 Play Mode 且授权有效时才执行；
     - 执行前消费授权，避免后续被自动/重复触发。
- 代码层验证：
  - `git diff --check -- Assets/Editor/NPC/CodexNpcTraversalAcceptanceProbeMenu.cs` 通过；
  - `python scripts/sunset_mcp.py validate_script Assets/Editor/NPC/CodexNpcTraversalAcceptanceProbeMenu.cs --count 20`
    - `manage_script_compat.validation.status=clean`
    - 总 assessment=`unity_validation_pending`（MCP `stale_status` 外部态，不是脚本编译错误）。
- 当前恢复点：
  - 后续任何 probe/live 测试都先等用户口头授权，再手动执行；
  - 主线回到 `Primary` 下 `NPC 002` 依赖与性能复判，但这一步当前先停。

## 2026-04-08｜导航检查V2：面向打包的导航稳定化大刀（不改业务语义）已提交

- 当前主线：
  - 用户要求“保持原设计不变”的前提下，彻底清扫 NPC/导航实现层漏洞，目标是降低卡顿、撞墙重算、原地转圈与重建抖动。
- 本轮子任务：
  - 在 `NPCAutoRoamController + NavGrid2D + NavigationPathExecutor2D + NPCMotionController` 做防错与降耗，不碰玩家/NPC目标语义。
- 本轮完成：
  1. `NPCAutoRoamController.cs`
     - 新增路径构建预算闸门与失败退避：同帧最多一次重建，连续失败按短退避回退，抑制 blocked 态高频重复重建。
     - `CheckAndHandleStuck()` 在 `TryRebuildPath` 成功后改为“本帧直接结束移动处理”，避免继续使用重建前 waypoint 导致抖动。
  2. `NavGrid2D.cs`
     - `IsPointBlocked(...)` 从多轮扫描改为单轮分类判定，减少重复 `ShouldIgnoreDynamicNavigationCollider` / 标签 / layer / explicit 列表扫描。
     - 同时去掉该路径里的一次冗余 walkable-override 额外查询，保持判定语义不变但降低物理查询放大。
  3. `NavigationPathExecutor2D.cs`
     - `TryBuildPath(...)` 改为事务式路径提交：先在临时路径构建，成功后再写回 `state.Path`；失败时不再把旧路径清空。
  4. `NPCMotionController.cs`
     - 新增朝向最小保持窗口 `facingDirectionMinHoldSeconds`，仅做动画朝向防抖，压“贴墙抖动时视觉转圈”。
- 代码验证：
  - `git diff --check` 通过（4 文件）。
  - `manage_script validate`：
    - `NavGrid2D` / `NavigationPathExecutor2D` = clean；
    - `NPCAutoRoamController` 仅保留工具 warning（字符串拼接提示）；
    - `NPCMotionController` 仅保留工具 warning（建议把刚体操作放 FixedUpdate）。
  - `sunset_mcp.py errors --count 120` = `0 error / 0 warning`。
  - `status` 仍有 external 旧 warning（TMP 过时 API，非本轮 owned）。
- 提交：
  - `main` 上已提交 `4c736a7a`
  - message=`harden navigation runtime: throttle path rebuild and reduce blocker query overhead`
  - files:
    - `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
    - `Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs`
    - `Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs`
...
- 当前恢复点：
  - 代码层大刀已完成且可回退；
  - 下一步只差用户授权后的最小 live 复判（Primary 下 NPC 群跑+桥水边缘），确认 runtime 体感闭环。

## 2026-04-08｜导航检查V2：已获授权并在 Town 现场完成双轮快速 live 复验

- 当前主线：
  - 用户授权“现在就跑”，本轮目标是快速验证大刀后 runtime 是否稳定，不改代码。
- 本轮执行：
  1. 在 `Town` 当前 Edit 现场进入 Play，并通过 `CodexEditorCommandBridge` 触发：
     - `MENU=Tools/NPC/Spring Day1/Run Runtime Targeted Probe`
  2. 连续跑 2 轮，均自动回到 Edit。
- 结果（两轮一致）：
  - `instance=7/7`
  - `selfTalk=2/2`
  - `informal=7/7`
  - `pair=2/2`
  - 唯一红点：`walkAway=1/2`，固定为
    - `101: walk-away timeout | mode=WaitingNpcReply | triggered=False | state=Idle | endReason=Completed`
- 运行态信号：
  - 本次 fresh console error 仅来自该 probe 自身 FAIL 输出（2 条），未出现新的导航编译/运行崩溃类红错。
- 当前判断：
  - 导航大刀后的运行态总体稳定，失败点收敛成单一 `walk-away` 子场景（更偏会话/交互时序），不是“NPC 全线导航失效”。

## 2026-04-08｜导航检查V2：walk-away 单点已修，runtime probe 已跑出全绿

- 本轮改动：
  - 文件：`Assets/Editor/NPC/SpringDay1NpcCrowdValidationMenu.cs`
  - 定点修复 `TickInterruptProbe()`：
    1. `WaitingNpcReply` 触发条件过窄时，增加同语义 fallback 触发窗口（已完成至少 1 轮 exchange 且仍在会话中）；
    2. 会话在触发前自然 `Completed` 时，允许按 spec 进行 1 次自动重试，避免直接 timeout 误红。
- 验证过程（Town 现场，授权 live）：
  1. 第一次复跑仍见旧红（`101 walk-away timeout`）；
  2. 强制 `Assets/Refresh` 后再次跑，`walkAway=2/2` 已通过；
  3. 中途出现过一次新的 `203 informal timeout`（`NpcTyping`），紧接下一轮复跑即恢复全绿。
- 最新稳定样本：
  - `PASS | instance=7/7 | selfTalk=2/2 | informal=7/7 | pair=2/2 | walkAway=2/2`
  - fresh console：`errors=0 warnings=0`
- 提交：
  - `7d5b753d`
  - `stabilize npc runtime targeted probe walk-away trigger window`
- 当前判断：
  - 导航主链问题已从“单点红”回到“可稳定跑绿”；剩余风险是该 runtime probe 仍有偶发波动（已观测 1 次 `203 informal timeout` 后自行恢复）。
  - 追加样本：又补跑 1 轮，继续 `PASS`，目前连续 2 轮全绿。
  - 当前 blockers：
    - `live-retest-pending-primary-scene-context`
    - `external-console-errors:NPCBubblePresenter SendMessage OnValidate`
    - `runtime-noise-history:repeated-snapshot-menu-requests-needs-clean-probe-window`

## 2026-04-09｜只读审计：Town/Primary 农田挡路与桥水失效的静态根因已压实（先查不落地）

- 当前主线：
  - 用户要求先查清“农田为什么会挡住、桥和水为什么过不去”，这轮只做场景 YAML + traversal 配置只读审计，不直接改代码。
- 本轮子任务：
  - 对读：
    - `Assets/000_Scenes/Town.unity`
    - `Assets/000_Scenes/Primary.unity`
...
- 本轮查实：
  1. `Town` 场景里确实存在：
     ...
     - `Layer 1 - Farmland_Water`
     - 其中 `Farmland_Water` 带 `TilemapCollider2D`。
  2. `Town` 的 `TraversalBlockManager2D` 当前落盘与“桥底座可走”的口径不一致：
     - `walkableOverrideTilemaps = []`
     - `walkableOverrideColliders = []`
     - 手填 `blockingTilemaps` 是 4 个挂在 `基础设施` 下的 tilemap chunk；
     - 手填 `blockingColliders` 里包含 `Farmland_Water` 与多张 `Layer 1 - Props`。
  3. `Town.unity` 里的 `TraversalBlockManager2D` 没有把新增自动收集字段落盘，所以运行时会直接吃脚本默认值：
  ...
  4. `TraversalBlockManager2D` 当前逻辑会把：
     - 名字带 `water` 的源归进 `soft-pass`
     - 名字带 `border / props / wall / tree ...` 的源自动补进阻挡来源
     - 且不会因为“这是用户没手填的 tilemap”就自动尊重现有 Inspector 语义。
  5. `FarmlandBorderManager` 会在运行时往 `farmlandBorderTilemap` 写 tile：
     - `OnCenterBlockPlaced()` 会 `SetTile(...)`
     - `ApplyBorderTile()` 会写 `farmlandBorderTilemap`
     - `RefreshAllBorders()` 会 `ClearAllTiles()` 后重刷所有边界。
  6. 结合上面几条，`Town` 的“农田会挡住我”当前最像：
     - `Farmland_Border` 原本在 scene 文件里是空 tilemap，
     - 但运行时被 `FarmlandBorderManager` 补砖，
     - 又因为 `TraversalBlockManager2D` 自动把 `border` 类 tilemap 纳入阻挡，
     - 最终形成了用户没有手工声明过的 traversal 阻挡。
  7. `Primary` 的 bridge/water 配置本身是落盘正确的：
     - `blockingTilemaps = Layer 1 - Water + Layer 1 - 桥_物品0`
     - `walkableOverrideTilemaps = Layer 1 - 桥_底座`
     - 但它同时也显式开着 `autoCollectSceneBlockingTilemaps = true`。
- 当前判断：
  - `Town` 农田挡路的第一责任点已经压实到 `TraversalBlockManager2D` 自动收集越权，而不是用户手工配置错。
  - `Primary` 的桥/水问题不能直接归咎于“桥底座没配”，因为场景落盘上这套桥配置是存在的；它更像是 runtime/build 差异或额外 traversal 条件问题，这部分本轮只静态定位到“不是桥配置缺失”。
- 验证状态：
  - `静态推断成立`
  - 未做 live / build 复测，遵守用户“这轮先查不直接落地”的要求。
- 当前恢复点：
  - 如果用户允许下一刀，最优先不是改玩家导航，而是先收 `TraversalBlockManager2D` 对手工场景配置的越权补收口：
    1. `Town`：不再让 `Farmland_Border` 这种运行时补砖 tilemap 被自动当成 traversal 阻挡；
    2. `Primary`：在保留桥/水显式语义的前提下，再单独查 build/editor 差异。

## 2026-04-09｜真实施工：让 traversal 严格服从场景最终配置，并抬高桥面显式可走优先级

- 当前主线：
  - 用户明确要求“场景里最终配置就是运行态/打包态真值”，本轮开始真实施工，目标只收 `Town` 农田挡路与 `Primary` 过桥配置不被尊重这两类 bug。
- 本轮切片：
  - `respect-scene-traversal-config-and-bridge-override`
  - `Begin-Slice` 已登记；收尾因 blocker 已 `Park-Slice`。
- 本轮实际改动：
  1. `Assets/YYY_Scripts/Service/Navigation/TraversalBlockManager2D.cs`
     - 新增 `HasAnyExplicitTraversalSourceConfigured()`。
     - 现在只要场景里已经显式配置了任一 blocking / walkable override 来源，`GetResolvedBlockingTilemaps()` 与 `GetResolvedBlockingColliders()` 就不再偷偷追加 scene auto-collect 结果。
     - 目的：让用户手工配置成为最终语义，阻断 `Farmland_Border` 这类运行时补砖 tilemap 被自动吃成 traversal 阻挡。
  2. `Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs`
     - `HasWalkableOverrideAt()` 现在先认“该 cell 上存在显式 walkable override tile”。
     - `IsPointBlocked()` 现在先做一轮显式 walkable override 探测，再决定是否让 explicit obstacle / soft-pass 源生效。
     - 显式 walkable override 存在时，会压过 traversal 这条链自己挂进来的 explicit obstacle / soft-pass 源；tag/layer 等通用阻挡语义不受这条修复影响。
     - 目的：让 `桥_底座` 这类显式可走覆盖在桥/水场景里拥有稳定优先级，而不是被 traversal 附带的阻挡源顶掉。
- 验证：
  - `manage_script validate TraversalBlockManager2D` = `clean`
  - `manage_script validate NavGrid2D` = `clean`
  - `git diff --check -- Assets/YYY_Scripts/Service/Navigation/TraversalBlockManager2D.cs Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs` 通过
  - `validate_script NavGrid2D` = `external_red`
    - `owned_errors = 0`
    - external blocker 为 `Assets/YYY_Scripts/Controller/Input/GameInputManager.cs` 的 `CS0165` 三条
    - 同时伴随 3 条外部 warning（TMP 废弃 API）
- 当前判断：
  - 本轮目标代码已落地，且我的 owned 脚本验证是干净的。
  - 当前不能 claim “可直接 sync / 全仓无红”，不是因为我这两刀还红，而是因为外部已有 `GameInputManager.cs` 编译红挡在前面。
- 当前 blockers：
  - `ready-to-sync-lock-timeout`
  - `external-red-GameInputManager-cs0165`
- 当前恢复点：
  - 下一步如果继续，不是再改这两刀，而是：
    1. 等或协调 `GameInputManager.cs` 外部红清掉；
    2. 再做运行态 / 打包态最小 bridge+farmland 复验。

## 2026-04-09｜真实施工：修跨场景 persistent player 的 traversal 重接线时序 bug

- 当前主线：
  - 用户确认 `Primary` 直进能过桥，但 `Town -> Primary` 或 `Primary -> Home -> Primary` 会失效；本轮改为只修跨场景 traversal 重绑，不再怀疑桥配置本身。
- 本轮切片：
  - `cross-scene-player-traversal-rebind-fix`
  - `Begin-Slice` 已登记；本轮停在待用户复测，收尾将 `Park-Slice`。
- 根因压实：
  1. `Primary` 直进能过，说明 `Primary` 的桥/水/override 场景配置本身成立。
  2. 真问题出在 `PersistentPlayerSceneBridge.ReapplyTraversalBindings(scene)`：
     - `DestroySceneDuplicatePlayers(scenePlayers)` 用的是 `Destroy(...)`，属于帧末销毁；
     - 同一帧里紧接着就会跑 traversal 重绑；
     - 这会让 `TraversalBlockManager2D.ResolveReferences()` 有机会重新抓到“马上要被销毁的场景内 PlayerMovement”，而不是 `DontDestroyOnLoad` 的 persistent player。
  3. 结果就是：
     - 直接从 `Primary` 起跑正常；
     - 但从 `Town`/`Home` 切进来后，manager 可能短暂绑到错误玩家，随后引用变 Missing，桥面 contract 虽然在 scene 里是对的，当前玩家却没真正吃到。
- 本轮代码改动：
  1. `Assets/YYY_Scripts/Service/Navigation/TraversalBlockManager2D.cs`
     - 新增 `BindRuntimeSceneReferences(NavGrid2D, PlayerMovement)`，允许桥接层在跨场景时显式指定“当前场景 NavGrid + 当前 persistent player”。
  2. `Assets/YYY_Scripts/Service/Player/PersistentPlayerSceneBridge.cs`
     - `ReapplyTraversalBindings(scene)` 现在先拿到当前 scene 的 `NavGrid2D`；
     - 再对每个 `TraversalBlockManager2D` 显式注入 `persistentPlayerMovement` 和 scene navGrid；
     - 然后才执行 `ApplyConfiguration(rebuildNavGrid: false)`。
- 预期效果：
  - `Town -> Primary`
  - `Primary -> Home -> Primary`
  - 这两类经过 `PersistentPlayerSceneBridge` 的切场景链，都不再依赖 `FindFirstObjectByType<PlayerMovement>()` 的同帧时序运气，而是强制绑定到 persistent player。
- 验证：
  - `python scripts/sunset_mcp.py validate_script TraversalBlockManager2D.cs PersistentPlayerSceneBridge.cs`
    - `owned_errors = 0`
    - `external_errors = 0`
    - `assessment = unity_validation_pending`
    - 原因：本机 Unity MCP 基线此刻拒连，CLI 只拿到代码层结果，没拿到 live 回执。
  - `git diff --check -- TraversalBlockManager2D.cs PersistentPlayerSceneBridge.cs` 通过。
- 当前判断：
  - 这刀没有推翻前面“显式配置优先 / bridge override 优先”的修复；那两刀处理的是 scene 配置真值，这一刀处理的是跨场景重绑时序，问题层级不同，不是走歪路。
- 当前恢复点：
  - 下一步最小复测只需要验证：
    1. `Town -> Primary` 过桥；
    2. `Primary -> Home -> Primary` 过桥；
    3. 不要求本轮再扩到新业务逻辑。

## 2026-04-09｜只读总审：导航最终设计应是“一套契约内核 + 多种目标来源”，当前最大偏差在 NPC driver 层

- 当前主线：
  - 用户要求先别继续补丁式乱修，而是先把“最终要什么导航、现在为什么老翻车、正确落地路线是什么”一次压实。
- 本轮性质：
  - 全程只读分析，不进入真实施工；不跑新的 `Begin-Slice`，沿用当前 `PARKED`。
- 本轮读到的核心链路：
  - `Assets/YYY_Scripts/Service/Player/PlayerAutoNavigator.cs`
  - `Assets/YYY_Scripts/Service/Player/PlayerMovement.cs`
  - `Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs`
...
- 本轮最终判断：
  1. 用户原始需求是对的：
     - 玩家 / NPC / 其他可移动单位应共享同一套 traversal contract；
     - 区别只该落在“谁给目的地”和“完成后做什么”，不该落在桥/水/边界/静态阻挡/NPC 软穿越这类基础语义。
  2. 当前代码只实现了“半统一”：
     - `TraversalBlockManager2D + NavGrid2D + NavigationTraversalCore` 基本承担了共享契约层；
     - 但 `PlayerAutoNavigator` 与 `NPCAutoRoamController` 在路径推进、repath、avoidance、blocked recovery、动画朝向上仍是两套不同 driver。
  3. 真正最偏离设计方向的，不是 scene 配置，也不是 bridge override 本身，而是：
     - NPC driver 把“移动决策 / 卡住恢复 / 局部避让 / 速度提交 / 朝向表现”搅在一个超重循环里；
     - 玩家 driver 也被 crowd 特判越堆越厚，但玩家至少还有 `PlayerMovement` 这层比较稳定的最终速度/朝向出口。
  4. 所以过去反复翻车的本质，不是需求错，而是我之前总在错误层级补口：
     - 把本该统一内核 / driver 边界的问题，当成某个 case 的 heuristic 参数问题去修。
- 审查结论：
  - 最终正确架构应固定成三层：
    1. `Scene Contract Layer`
       - `TraversalBlockManager2D`
       - `NavGrid2D`
       - 场景 Inspector 真值、blocking / override / bounds
    2. `Shared Traversal Kernel`
    ...
    3. `Driver Layer`
       - 玩家：右键输入 / 跟随 / 到达判定
       - NPC：漫游目标采样 / 行为状态机
       - 这一层只应该负责“给目标”和“消费统一移动决策结果”，不应该各自长出一整套不同的阻挡解释学
  - 当前最需要重构/收敛的是第 3 层，尤其是 NPC。
- 最大问题点压缩：
  1. `NPCAutoRoamController` 太重：
     - `TickMoving()` 同时负责 waypoint、avoidance、stuck、rebuild、blocked recovery、velocity commit；
     - 历史上也正是这条链在卡顿和鬼畜里最容易雪崩。
  2. NPC 朝向当前是“观测速度驱动”而不是“最终承诺移动向量驱动”：
     - `NPCMotionController` 会在 observed velocity / external velocity / rigidbody velocity 之间切；
     - 只要 avoidance/repath 频繁抖，朝向就会跟着抖，产生一步三回头。
  3. 玩家 crowd 修法过度堆在 `PlayerAutoNavigator`：
     - 玩家现在之所以还能站住，是因为 `PlayerMovement` 仍提供了统一最终速度/朝向出口；
     - 但 `PlayerAutoNavigator` 已经堆了太多 scenario-specific passive NPC heuristics，继续这样补会越来越不可控。
- 最终落地方向：
  - 不重写整套导航，不改业务需求；
  - 只做“共享内核更硬、driver 更薄、NPC 朝向只认最终移动向量、rebuild/avoidance 做统一节流”。
- 验证状态：
  - `静态推断成立`
  - 尚未进入这轮实施。

## 2026-04-09｜导航检查：Profiler 三图与源码已对齐，Town 峰值卡顿主因落在 NPC 自漫游坏链路

- 当前主线：
  - 用户要求先别继续乱补，先把 `Day1 围观剧情后必现卡顿 + 跟村长时 Town 峰值` 的真实原因、最快修法、全修方案和代价差说清。
- 本轮性质：
  - 全程只读分析；未进入真实施工；不新跑 `Begin-Slice`，线程保持 `PARKED`。
- 最终判断：
  1. 峰值主因已经压到 `NPCAutoRoamController` 的 autonomous roam 主链，不是剧情移动本身。
  2. “剧情里正常 / 散会回 anchor 正常 / 自漫游发疯”的差别也压实了：
     - scripted move / anchor return 走的是更直接的外部速度提交链；
     - 自漫游走的是 `TickMoving -> stuck/rebuild/blockedAdvance/sharedAvoidance` 的重循环。
  3. 用户对“封闭区域不该被硬走、撞墙不该无限重试”的判断是对的：
     - 现在不是地图配置天然错；
     - 是 NPC 漫游 driver 没把“不可达 / 长时间无进展 / 封闭区域目标失效”及时收成失败语义，导致重复 rebuild / physics query / avoidance 抖动。
  4. Profiler 读数与静态代码链完全对上：
     - 图 2：`NPCAutoRoamController.Update()` 单帧约 `23ms`，`Physics2D.OverlapCircleAll≈1293`，`GC.Alloc≈0.7MB`
     - 图 3：同链路峰值约 `1339ms`，`OverlapCircleAll≈89767`，`GC.Alloc≈15.3MB`
     - 说明不是“偶发慢一点”，而是坏 case 下的高频物理查询 + GC 雪崩。
- 两档方案判断：
  1. 最快止血方案：
     - 预计 `0.5 ~ 1 天`
     - 只刀 `NPCAutoRoamController`，必要时极小补 `NPCMotionController`
     - 目标是先把峰值卡顿和撞墙硬走压掉，不做完整统一
     - 核心动作：
       - 封闭/无效目标快速失败：连续 occupy/advance 失败后直接 short pause + 重采样，不再连环重试
       - rebuild/backoff 去重：同目的地、同位置、同 blocker 窗口内只允许一次 heavy rebuild
       - 强制每渲染帧最多一次 heavy decision，阻挡态也不能放开疯算
       - 朝向改为只认最后提交给 motion 的移动向量，不再跟 observed velocity 抖
  2. 全部修复方案：
     - 预计 `2 ~ 4 天`
     - 约为最快止血的 `3x ~ 4x`
     - 目标是把 NPC driver 真正收薄，向“同一 traversal contract，多种目标来源”靠拢
     - 核心动作：
       - 把 blocked/no-progress/rebuild/detour 生命周期从 NPC 漫游里抽薄
       - `NPCMotionController` 彻底改成 committed move vector / facing 消费者
       - 保留 `NavGrid2D / TraversalCore / PathExecutor` 这套共享契约内核，不推翻玩家当前版本
- 风险与取舍：
  - 最快止血的代价：
    - NPC 会先变得更保守，可能更容易短暂停一下再重采样；
    - 避让灵性短期不会是最佳，但可以先止血，不再把 Town 帧率打穿。
  - 全部修复的代价：
    - 改动面更大，回归验证量更高；
    - 但方向最符合“统一契约、多种目标来源”，后面不容易继续 patch 崩坏。
- 验证状态：
  - `静态推断成立`

## 2026-04-10｜导航检查：NPC 自漫游峰值卡顿 demo 止血刀已落地，并补到最小 live probe

- 当前主线：
  - 只收 `NPC 自漫游坏 case demo 级止血刀`，并先把“现有节流为何失效”和“OverlapCircleAll 与当前 live 源码为何对不上”钉死。
- 本轮真实施工：
  1. `Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs`
     - 保留现有节流框架，不再误报成“完全没有止血”；
     - 新增 `NPC_BAD_CASE_STOP_DECISION_REUSE_SECONDS = 0.12f`，把 stop decision reuse 从“只够同一渲染帧”扩成“坏 case 短窗内可复用”；
     - 明确压实当前现有节流为何失效：
       - `PHYSICS_HEAVY_DECISION_REUSE_SECONDS = 0.02f` 只够极短窗口；
       - `CanReuseHeavyMoveDecisionThisFrame(...)` 在 `blockedAdvanceFrames / sharedAvoidanceBlockingFrames / hasDynamicDetour / pendingMoveCommand` 这些坏 case 下直接禁用；
       - `TryAcquirePathBuildBudget()` 只挡同帧重复 build，不挡跨帧 storm；
       - `PATH_BUILD_FAILURE backoff` 只在 build 失败时生效，build 成功但仍撞墙的坏 case 会立刻清空 backoff；
       - `BLOCKED_ADVANCE_RECOVERY_COOLDOWN` 只包 `TryHandleBlockedAdvance`，`CheckAndHandleStuck` 仍可绕开。
     - 止血刀本体：
       - 坏 case 下延长 stop decision reuse；
       - `blockedAdvanceFrames >= 3` 且仍属 autonomous roam 时，直接结束当前 move cycle，不再继续 `TryBeginMove` 硬重采样；
       - stuck 已升级到 cancel 时，autonomous roam 直接 short pause，不再立刻再开一轮重采样。
     - 为 live probe 暴露最小只读指标：
       - `DebugBlockedAdvanceFrames`
       - `DebugConsecutivePathBuildFailures`
       - `DebugPathBuildCooldownRemaining`
     - 新增 stopgap 命中标记：
       - `BlockedAdvanceStopgap`
       - `PassiveAvoidanceStopgap`
       - `StuckCancelStopgap`
  2. `Assets/YYY_Scripts/Controller/NPC/NPCMotionController.cs`
     - 朝向优先改为消费 `external velocity`，不再优先跟随 noisy observed velocity；
     - 目的是让 NPC 朝向更贴近“最后提交的移动决策”，而不是 blocked/no-progress 时被观测速率拖偏。
  3. `Assets/Editor/NPC/NpcRoamSpikeStopgapProbeMenu.cs`
     - 新增本地命令桥可执行 probe：
       - 菜单 `Tools/Sunset/NPC/Run Roam Spike Stopgap Probe`
       - 输出 `Library/CodexEditorCommands/npc-roam-spike-stopgap-probe.json`
     - 记录 `npcCount / roamNpcCount / maxGcAllocBytes / blockedAdvanceStopgapSamples / stuckCancelStopgapSamples / topSkipReasons`。
- `OverlapCircleAll` 与当前 live 源码差异的最终裁定：
  - 当前更可信的是 `B：用户那次 profiler 对应的是旧 build / 旧二进制现场，而不是当前 live 源码主链`
  - 证据：
    - 当前 `NavGrid2D.IsPointBlocked(...)` 主链已是 non-alloc `Physics2D.OverlapCircle(..., cache)`；
    - `git show 0005602b:NavGrid2D.cs` 仍是 `OverlapCircleAll`；
    - `git show ee7401f6:NavGrid2D.cs` 已切到当前 array/cache 版；
    - 当前 NPC / Navigation 主链里未再搜到 `OverlapCircleAll`。
  - 补充说明：
    - 当前 live 代码仍可能通过 `Array.Resize` 和重复 build/query 产生 GC；
    - 但这和 profiler 里“主链直接显示 OverlapCircleAll”不是同一回事。
- live 取证：
  - 代码层：
    - `validate_script` 目标 3 文件，`owned_errors=0 / external_errors=0 / assessment=unity_validation_pending`
    - `git diff --check` 通过
  - 本地命令桥 live probe：
    - `Town` 现场已真实跑完 `NpcRoamSpikeStopgapProbe`
    - 结果文件：`Library/CodexEditorCommands/npc-roam-spike-stopgap-probe.json`
    - 当前结果：
      - `npcCount=25`
      - `roamNpcCount=16`
...
- 当前判断：
  - 这份 live probe 说明：
    - “Town 裸 roam 现场”当前没有复现你说的坏 case；
    - 所以这次 spike 确实不是所有 roam 场景都炸，而是更像 `Day1 围观后 + 跟村长继续走` 那段特定时序坏序列。
  - 因此这轮可以真实 claim 的是：
    - 代码止血刀已落；
    - 现有节流失效点已钉死；
    - `OverlapCircleAll` / live 源码差异已钉死；
    - 但“是否已经压住你那条 day1 坏序列”仍待该序列 live 复判。
- thread-state：
  - `Begin-Slice = 已跑`
...
## 2026-04-11 只读复盘：朝向已收住后，Town 当前“顶墙 / 避让像没了”的第一判断
- 当前主线目标：
  - 用户最新主诉已经切成：朝向基本对了，但 NPC 仍会撞墙，想知道这是不是“静态导航根本不存在”，以及下一刀能不能做到“功能和性能兼有”。
- 本轮只读核对的关键事实：
  1. `Town` 当前并不是没有静态导航。
     - [Town.unity](D:/Unity/Unity_learning/Sunset/Assets/000_Scenes/Town.unity#L151763) 的 [NavGrid2D](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/NavGrid2D.cs) 仍保留 obstacle tags
     - [Town.unity](D:/Unity/Unity_learning/Sunset/Assets/000_Scenes/Town.unity#L179252) 的 [TraversalBlockManager2D](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/TraversalBlockManager2D.cs) 仍在运行时把 NavGrid/Player/NPC traversal 绑定回 scene
  2. `Town` 现在是“显式 source 白名单模式”，不是“自动全收”。
     - [TraversalBlockManager2D.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/TraversalBlockManager2D.cs#L465) 的 `HasAnyExplicitTraversalSourceConfigured()` 一旦为真，就会让 auto collect 收窄
     - 当前 `Town` 里 `blockingTilemaps / blockingColliders` 都不为空，`autoCollectSceneBlockingColliders=0`
     - 所以后续新增静态 collider/tilemap 不会再自动进 nav block source；必须显式补进列表
  3. 但就当前 `Town` 已落盘 YAML 来看，这份白名单并不是空的，也不是完全失效：
     - manager 里确实挂着 `Layer 1 - Wall`、`Layer 1 - Props` 系列、`Layer 1 - Farmland_Water`，以及 4 组 composite tilemap source
     - 因此“静态导航根本不存在”这个判断不成立
  4. `shared avoidance` 也不是彻底没了，只是它本来就不负责墙：
     - [NPCAutoRoamController.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs#L1838) 的 `TryHandleSharedAvoidance(...)`
     - [NavigationAgentRegistry.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/NavigationAgentRegistry.cs#L32)
     - [NavigationLocalAvoidanceSolver.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/NavigationLocalAvoidanceSolver.cs#L253)
     - 这条链只解动态单位冲突，不解墙、房子、树、杆子的静态阻挡
  5. 所以当前坏相更像“静态阻挡 / 目标点 / 自漫游半径三层没对齐”，不是“整套避障系统消失”。
- 001~003 与其它常驻为什么分化：
  1. 001~003 与 101~301 当前 `sharedAvoidanceLookAhead / repathCooldown / avoidanceRadius / priority` 参数是一致的
  2. 真正差异落在 `activityRadius`：
     - 001=`1.7`
     - 002=`1.85`
     - 003=`1.55`
     - 101~301 普遍=`3`
  3. 这说明 001~003 看起来“更稳”，更像是活动范围更小、坏目标采样更少，不是因为它们独占了另一套更好的避障逻辑
  4. 小动物 prefab 也沿用同一套自漫游/avoidance contract，只是半径不同，因此大概率同根
- 还没完全钉死但当前最像的第一真问题：
  1. `Town Runtime Anchor Readiness Probe` 只验证了 anchor/slot/home-anchor 的“存在 + 在 bounds 内”
  2. 它没有验证这些点对 [NavigationTraversalCore.CanOccupyNavigationPoint(...)](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/NavigationTraversalCore.cs#L144) 是否真可站
  3. 再结合：
     - [NPCAutoRoamController.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs#L2270) 的 autonomous roam 采样
     - [NPCAutoRoamController.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Controller/NPC/NPCAutoRoamController.cs#L2309) 的 destination correction limit
     - [NavigationTraversalCore.cs](D:/Unity/Unity_learning/Sunset/Assets/YYY_Scripts/Service/Navigation/NavigationTraversalCore.cs#L240) 的 occupiable destination resolve
  4. 当前最像的现实问题是：
     - `HomeAnchor / DailyStand / ResidentSlot / roam center` 里有一部分点贴墙或落在 blocked edge 邻域
     - 大活动半径常驻 NPC 更容易采到这些坏点
     - 然后 shared avoidance 又无法替代静态墙体合法化，于是表现成“方向对，但一直顶墙”
- 历史窗口重新确认：
  - 能看见历史上用户确实给过“NPC 避障已到可用地步”的正反馈，不是记错
  - 所以下一刀目标应是“恢复那个够用窗口的功能体验，同时保住 4/5 之后的性能止血”，而不是大回滚
- 当前恢复点：
  - 下一刀最值钱的施工目标已经收窄为：
    1. 先做坏 NPC 组 `occupiable target` 验证
    2. 再补最小目标修正 / roam 采样收紧
    3. 最后才看 shared avoidance 是否还要追回一点动态让行
  - 这条路线可以保住当前性能 hardening；不需要回滚 `TryAcquirePathBuildBudget / stop decision reuse / path build backoff`
