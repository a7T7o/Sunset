...
### 会话 8 - 2026-03-18（第一次唤醒复盘：暴露 shared root 分支租约缺口）
**用户目标**：
> 读取 `第一次唤醒` 目录中的五线程真实回包，判断第一次唤醒为什么仍然出事、问题到底出在哪、是否需要立项新阶段继续补闸机。
...
2. 识别出第一次唤醒的真实表现分层：
   - `NPC`：遵守只读比较，没有切分支
   - `spring-day1`：遵守只读预检，没有切分支
   - `farm`：在第一次唤醒阶段提前执行了 `ensure-branch`，把 shared root 切到 `codex/farm-1.0.2-cleanroom001`，随后在用户指出后切回 `main`
   - `导航检查`：自称未切分支，但其只读审计运行时 live 现场已被 farm 切到 farm 分支，因此结论被污染
   - `遮挡检查`：只读审计相对干净，结果基本可信
3. 锁定新问题本质：
   - 阶段 20 已经拦住了“错分支提交 / 错 owner sync”
   - 但还没拦住“第一次唤醒阶段就提前切分支”
   - 在 shared root 模型里，`切分支` 本身就是全局写态，不应被当成局部只读前置动作
4. 明确当前 live Git：
   - `D:\Unity\Unity_learning\Sunset @ main`
   - 当前新增的唯一 dirty 是本次真实回包目录：
     - `20_shared-root现场回正与物理闸机落地/第一次唤醒/`

**关键决策**：
- 第一次唤醒 prompt 的结构仍然过于宽松，尤其是把“第一次只读复核”和“允许进入 branch-task”混在同一轮话术里。
- 后续若继续治理，必须新开阶段补：
  - shared root 分支租约闸门
  - 第一次唤醒与第二次“获准进入任务分支”的两阶段协议
  - `ensure-branch` 的显式 claim / grant 机制
- 这不是要推翻阶段 20，而是说明阶段 20 解决了 `sync/commit` 级问题，还没解决“branch switch 也是共享写态”这件事。

**恢复点 / 下一步**：
- 建议立项 `21`，专门处理“第一次唤醒复盘与 shared root 租约闸门”。
- 在 `21` 完成前，第一次唤醒只允许只读，不再在同一条 prompt 里夹带条件式 `ensure-branch`。
