...
- 下一步如果继续推进，应直接转入：
  - `001部分回执.md` 定点修复
## 2026-03-23｜MCP 根因继续下钻到“旧线程缓存未刷新”
**当前主线目标**
- 不只把 `unityMCP + 8888` 立成静态基线，还要把“旧线程 / 旧会话会继续沿用旧 MCP 路由缓存”写进 live 规则，避免线程再次把旧口径误判成服务端回滚。

**本轮完成**
1. 只读确认 shared root `main` 下：
   - `config.toml` 已回正到 `http://127.0.0.1:8888/mcp`
   - `check-unity-mcp-baseline.ps1` 已返回 `baseline_status: pass`
2. 复核 Codex 会话日志后，确认同一条旧线程在已经读到 `8888` 配置后，后续请求仍继续打旧端口，根因已从“单纯配置漂移”收紧到“旧会话 MCP 路由缓存未刷新”。
3. 已把这条口径同步到：
   - `AGENTS.md`
   - `Sunset当前规范快照_2026-03-22.md`
...
4. 已把当前 active memory 里剩余的旧端口字面改写成“旧端口口径（已失效）”，避免它们继续被读成 live 事实。

**关键决策**
- 今后只要 `config + 8888 + pidfile + baseline` 已通过，但旧线程仍报旧端口 / 旧桥名 / `resources/templates` 为空，优先判为“旧会话 MCP 路由缓存未刷新”。
- 继续验证时，优先开新线程 / 新会话，或先手工对 `8888` 做 initialize + `tools/list`，不要直接回退到“服务端又坏了”的旧判断。

**恢复点 / 下一步**
- 这条根因治理已经从聊天结论落进 live 规则与 prompt。
- 下一步只剩白名单收口，并继续观察真实线程是否还会复发同类误判。

## 2026-03-23｜用户手改 Anvil meta 已按单文件白名单提交
**当前主线目标**
- 用户明确说明 `Anvil.png.meta` 是自己手动做的修改，并要求我直接帮他提交；同时顺手追查 4 个 TMP 字体资源是否真能归到 `spring-day1`。

**已完成**
1. 已核对 `Anvil.png.meta` 的真实 diff：
   - 不是空脏改
   - 主要是新增 `physicsShape`，并把多处 `tessellationDetail` 从 `-1` 调整为 `0`
2. 已按单文件白名单同步提交：
   - 文件：`Assets/ZZZ_999_Package/Pixel Crawler/Environment/Structures/Stations/Anvil/Anvil.png.meta`
   - 提交：`30d6b5ca`
3. 已追查 4 个 TMP 字体资源的最近 Git 历史：
   - 最近提交只看到 `41caa67b`、`db648ed0`
   - 没有看到任何 `2026.03.23_spring-day1_*` 提交直接触碰这 4 个资产

**关键决策**
- 用户自己的临时修改也可以提交，口径和线程一样：
  - 视作“一刀自己的最小白名单 checkpoint”
  - 只收自己这次明确想收的路径
  - 不夹带仓库里其他线程的 dirty
- 当前这 4 个 TMP 字体资源还不能直接按 `spring-day1` 追责：
  - Git 证据只支持“它们现在是本地 dirty”
  - 不支持“它们已经被 spring-day1 最近提交正式带入”
  - 更像是 Unity / TMP 本地重序列化副产物，或某次字体相关现场操作留下的未提交改动

**恢复点 / 下一步**
- 如果用户下一步要继续清现场，最自然的动作应是：
  - 要么继续定点追查这 4 个 TMP 字体资源的产生时机
  - 要么先让真正触碰字体 / DialogueUI 的线程自行白名单收口
