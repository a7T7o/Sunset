...
- [ ] 15. 设计 `dirty` 分级与清扫推送机制
  ...
  当前进展：已形成 `02_专题分析/2026-03-21/dirty分级与takeover边界设计稿.md`，并已在 `scripts/git-safe-sync.ps1` 落下首轮 dirty 分级报告层（`DirtyLevel / OwnerHint / PolicyHint`）；2026-03-21 的三轮真实样本回放已连续修正分类与 owner 边界：第一轮补齐了 `Primary.unity` 大小写漏判与 `Assets/222_Prefabs/*`、`Assets/111_Data/*`、`Assets/100_Anim/*.controller*`、`Assets/Sprites/*.meta` 的误报 `D2`；第二轮又确认并修正了所有 `.unity / .unity.meta` 场景文件、`Assets/100_Anim/*` 下真实 `.anim` clip、`Assets/Sprites/*` 下真实 sprite 文件仍被误报 `D2` 的问题，同时修正了 `HotbarSelectionService.cs` 的 owner hint 误归治理线程；第三轮继续补齐了 `spring-day1` 被误认成 `scene-build`、以及 `BatchAddOcclusionComponents.cs / OcclusionManager.cs / OcclusionTransparency.cs / OcclusionSystemTests.cs` 等遮挡相关文件误回退到治理线程默认 owner 的问题。当前结论仍为“先分级、先报告、默认硬闸门不撤，且暂不批准跨线程 raw dirty takeover”。
...
- [x] 21. 固化恢复开发总控规范与线程级放行 prompt
  说明：把“现在谁能开发、谁要等待、什么时候必须先回执再发下一条”从聊天口径固化成正式文件，并为 `scene-build / NPC / 农田交互修复V2 / 导航检查 / 遮挡检查 / spring-day1` 生成当前可直接分发的 prompt。
  当前进展：已新增 `00_工作区导航/恢复开发总控与线程放行规范_2026-03-21.md` 作为当前总控单一事实源，并新增 `01_执行批次/2026.03.21_恢复开发总控_01/` 目录，落下治理线程职责说明、统一最小回执格式，以及 `scene-build / NPC / 农田交互修复V2 / 导航检查 / 遮挡检查 / spring-day1` 六份当前版 prompt；同时新增 `Codex规则落地/2026-03-21_批次分发_09_恢复开发总控与线程放行规范.md` 作为治理入口，并补齐 `治理线程_当前执行Prompt.md`，让治理线程自己也有显式执行入口，而不再只有职责说明。
