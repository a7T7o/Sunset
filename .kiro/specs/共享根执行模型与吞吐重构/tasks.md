...
- [x] 21. 固化恢复开发总控规范与线程级放行 prompt
  ...
  当前进展：已新增 `00_工作区导航/恢复开发总控与线程放行规范_2026-03-21.md` 作为当前总控单一事实源，并新增 `01_执行批次/2026.03.21_恢复开发总控_01/` 目录，落下治理线程职责说明、统一最小回执格式，以及 `scene-build / NPC / 农田交互修复V2 / 导航检查 / 遮挡检查 / spring-day1` 六份当前版 prompt；同时新增 `Codex规则落地/2026-03-21_批次分发_09_恢复开发总控与线程放行规范.md` 作为治理入口，明确后续如线程回执改变风险、顺序或边界，治理线程必须先更新这些文件再通知用户。
