...
- 问题不在“它有没有业务结论”，而在“它做完后没有顺利退场”。

### 3.2 真正故障点
- `ensure-branch` 执行过程中曾出现系统级异常。
- 但从 live 现场可确认：
...
- 对应逻辑位于：
  - [git-safe-sync.ps1](/D:/Unity/Unity_learning/Sunset/scripts/git-safe-sync.ps1#L299)
  - [git-safe-sync.ps1](/D:/Unity/Unity_learning/Sunset/scripts/git-safe-sync.ps1#L2140)
- 但异常发生后，这个状态没有可靠落成，于是：
...
- 责任：
  - `NPC` 线程负执行责任。
  - 治理线程负验收责任。
- 根因：以前只验 `carrier-ready`，没有把 `main-ready` 设成硬门。
...
- 这类故障如果不修，会导致：
  - 线程误以为“业务没做对”
  - 用户误以为“又是某个线程乱搞”
  - 实际上却是 shared root 状态机断链

## 6. 修复目标
### 6.1 脚本层
- 补齐 `ensure-branch` 的异常后状态一致性：
  - 要么完全失败且不切分支
  - 要么一旦切入成功，就必须可靠落成 `task-active`
- 为 `sync / return-main` 增加“半成功残留态”的识别与兜底。
- 禁止线程在系统异常后继续自行做环境手术。
...
- prompt 模板补入明确红线：
  ...
  - 立即回报治理线程
- docs-only fallback 需要更早出现，而不是等线程自己绕了一大圈才退回最小收口。
...
1. 业务推进优先：
   - 先继续 `农田交互修复V2`
2. 治理修复并行准备：
   - 以本文件为入口修 `scripts/git-safe-sync.ps1`
3. 第二波业务继续前再判断：
   - `导航`
   - `遮挡`
