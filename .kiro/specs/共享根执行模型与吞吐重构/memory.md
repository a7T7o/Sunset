...
## 2026-03-25｜通用前缀已补入“stop-early”与“验收交接”字段

- 当前主线目标：
  - 用户明确要求把“live 取够证据就停”和“线程到用户终验阶段必须交验收包”推进到通用线程前缀，而不是继续靠治理线程临场提醒。
- 本轮已完成：
  - 更新：
...
- 本轮新增稳定事实：
  1. 普通线程现在在前缀层就会被要求：
     - 进入 Unity / MCP live 前先写清证据目标、最大轮次和停机信号
     - 一旦拿到证据立刻 `Pause / Stop`
     - 回到 `Edit Mode`
  2. 如果线程已经推进到“接下来要用户终验”，前缀层也会直接要求它：
     - 先做 compile / test / MCP 自验
     - 再给验收步骤、预期结果、失败判读和回执单
  3. 通用聊天字段也已增加：
     - `验收指南路径 / 验收回执单`
- 当前恢复点：
  - 后续治理线程若继续直接使用这份前缀，不应再收到“只说等用户测”或“长时间不收 Play”的低质量回执。

## 2026-03-23｜批次活文档 `001部分回执.md` 已完成定点去乱码修复
**当前主线目标**
- 把当前批次中仍然处于活跃链路、且已确认真实损坏的 `001部分回执.md` 修回到可继续阅读、可继续接手的状态。

**本轮完成**
1. 已确认该文件此前不是单纯终端假乱码，而是确实混入了替换字符与失真的中文段落。
2. 已定点重建 5 段损坏回执：
   - 1 段 NPC 第二刀回执
   - 4 段遮挡检查回执
3. 这些段落的恢复依据分别来自：
   ...
   - 已保留的真实验证结果与证据路径
4. 已复检：
   - `001部分回执.md` 当前 `HAS_UTF8_REPLACEMENT=False`
   - 文件已不再含替换字符残留

**关键决策**
- 对仍在活跃链路中被直接引用的回执文档，不能把“历史损坏”继续留着。
- 修复策略不是盲目转码，而是：
  - 能从原始线程记忆找回的，用真实语义修回
  - 找不到逐字原文的，用当前证据重建成干净、可执行的回执

**恢复点 / 下一步**
- `001部分回执.md` 现在已经不再算“待修坏样本”。
- 如果后续继续做编码治理，下一步应转去其他仍处于活跃链路且真实损坏的文档。
