# 农田交互修复V2｜执行层 smoke test 01

```text
【执行层 smoke test 01｜农田交互修复V2】
...
- 进入后只做只读核对：
  ...
  - 你自己的 1.0.2 carrier 仍在位
- 不写业务代码
- 不写 tracked 回执
- 核对完成后立即 return-main

先执行：
1. powershell -ExecutionPolicy Bypass -File C:\Users\aTo\.codex\tools\sunset-git-safe-sync.ps1 -Action request-branch -OwnerThread "农田交互修复V2" -BranchName "codex/farm-1.0.2-cleanroom001" -CheckpointHint "smoke-farm-readonly" -QueueNote "smoke-test-01"
