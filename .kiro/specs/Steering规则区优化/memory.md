...
## 2026-04-02（父层补记：当前规范快照已新增 `6C. 看守长模式`，并明确 `看守长 != 典狱长`）
- 当前父层主线：
  - 用户拿真实失败样本证明：Sunset 业务线程在听到 `看守长` 后，会滑回 `典狱长` 口径向用户索要回执材料；这说明先前的全局落地还不够，必须把项目快照也补成硬规则。
- 本轮新增稳定事实：
  1. 当前快照已经新增 `6C. 看守长模式`
  2. 当前快照已明确：
     - `看守长` = 线程完工后的用户验收交接
  ...
  3. 当前快照已写死：
     - 如果是在 Sunset 业务线程自己的对话里触发 `看守长`，默认就以当前线程自己最近完成或准备交接的这一刀为输入
     - 不允许先让用户再贴线程回执 / prompt 文件 / 线程名
- 父层恢复点：
  - 以后再审 Sunset 线程的“看守长是否真落地”，优先先看当前快照的 `6C` 有没有被执行，而不是再回到口头解释层

## 2026-04-02（父层补记：当前快照里的 `6C. 看守长模式` 已继续补强到“上一轮刚完成任务 + 首屏直入正文”）
- 当前父层主线：
  - 用户进一步纠正：问题不只在 `看守长 != 典狱长`，还在 `看守长` 必须直接接上一轮刚完成的任务，而且第一屏就该像 farm 那样进入可验正文，不该先讲模式前摇。
- 本轮新增稳定事实：
  1. 当前快照里的 `6C. 看守长模式` 已明确：
     - 默认输入是“上一轮刚完成且已汇报的那一刀”
     - 如果用户是在完成汇报后直接喊 `看守长`，线程必须直接沿刚才那轮完成项生成验收包
  2. 当前快照也已明确：
     - 首屏不应先讲模式切换或治理说明
     - 应直接进入验收交接正文
- 父层恢复点：
  - 后续再看 Sunset 业务线程的看守长样本时，除了查“有没有审自己”，还要查：
    1. 有没有锚到上一轮刚完成的任务
    2. 第一屏是不是已经直接进入验收正文
  - 若旧线程仍沿旧口径开口，已有可直接转发的热修补丁：
