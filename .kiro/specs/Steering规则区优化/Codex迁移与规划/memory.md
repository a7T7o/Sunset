...
### 会话补记 2026-03-11（用户再次强调主线锚定）
- 用户明确提醒：不要只盯着 Git 或分支操作本身，更要始终记住当前主线和最初目的。
- 当前这条线的主线不是“把 Git 玩明白”，也不是“到处切分支”，而是：为 Sunset 建立稳定可控的 Codex 工作底盘，确保关键工作内容不再丢失、线程不再漂移、后续业务线能在正确分支上安全连续推进。
- 当前正在处理的只是一个支撑子任务：把恢复后的根工作树整理到可顺畅切换分支的状态，服务于 NPC 线与其他工作线恢复正常推进。
- 后续每一步都要回到这个口径：先保住内容，再恢复秩序，最后让业务主线继续，而不是为了 Git 操作本身偏离目标。
