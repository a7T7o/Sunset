...
| T68 | ⏳ 待执行 | 建立 Sunset 线程承接审计表，把线程 id、线程名、标题、cwd、git_branch、工作区根、线程记忆、期望 worktree / 分支统一映射。 |
| T69 | ⏳ 待执行 | 设计并落地机器可读的线程路由映射源，供后续脚本与守护器读取。 |
| T70 | ⏳ 待执行 | 新建客户端状态审计脚本，系统检查 `state_5.sqlite`、`session_index.jsonl`、`.codex-global-state.json`、真实 worktree 分支的一致性。 |
| T71 | ⏳ 待执行 | 明确哪些线程状态字段允许自动纠偏，哪些只能提示不能改，避免为了“看起来一致”破坏线程历史语义。 |
| T72 | ⏳ 待执行 | 决定是扩展现有 `provider-bridge` 还是新建第二桥，把兼容层从“provider / cwd 修复”升级到“线程/workspace/UI 承接修复”。 |
