...
**状态**: 🟡 二轮协议扩写、脚本修复与可信基线重跑已完成（2026-03-12）
...
- `Codex统一协议建设最新现场基线_2026-03-12_75276fda.md`
- 必要时补充后续协议入口文档
...
- 只读基线已切换为 `Codex统一协议建设最新现场基线_2026-03-12_75276fda.md`
- 三项边界冲突的优先级已重判为：
...
**第三轮收口产物（2026-03-12）**:
- `scripts/codex_protocol_baseline.py` 已修复 `active-workspace-roots` 动态判定 bug，`classify_active_workspace()`、`rejudge_boundary_conflicts()` 与 `render_markdown()` 已统一使用实时 `active_assessment` 口径。
- 新的当前可信执行基线已重跑为 `Codex统一协议建设最新现场基线_2026-03-12_75276fda.md`。
- `Codex统一协议建设最新现场基线_2026-03-12_e70b1723.md` 已降级为“脚本修复前的失真样本”，仅保留历史取证价值。
- `.pyc` 口径已纠正：`9c723fe9` 是删除 `scripts/__pycache__/codex_thread_routing_audit.cpython-311.pyc`，`75276fda` 是把它重新加回；本轮已重新移除该缓存，并以最小范围忽略 `scripts/__pycache__/`。
