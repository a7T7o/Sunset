...
- 由于四份家访材料与 `对话丢失修复` 总记忆已经证明：旧版 `T73 / T74 / T75` 仍停留在中层状态修补，本阶段从 2026-03-12 起**接管**旧版 `T73 / T74 / T75` 的执行口径。
- 上文旧版 `T73 / T74 / T75` 保留为历史快照，不删除；但从本阶段开始，后续验收与推进一律以下面的“重构定义”为准。
- 本阶段明确区分四类任务：
  - 协议建设任务
  - 技术落地任务
  - 验证任务
...
| 分类 | 任务 | 状态 | 目标 |
|------|------|------|------|
| 协议建设 | T73（重构版） | ⏳ 待执行 | 把“激活工作区根纠偏”升级为统一现场判定协议的首轮落地。 |
| 验证任务 | T74（重构版） | ⏳ 待执行 | 把“三类线程回归”升级为按用户真实验收现场驱动的闭环验证。 |
| 文档/回滚 | T75（重构版） | ⏳ 待执行 | 把“回滚口径”升级为完成态分级、成果分层、回滚口径与对外话术统一收口。 |
| 技术落地 | T77 | ⏳ 待执行 | 在协议明确后，再收敛最小技术动作，把守护式纠偏限制在安全边界内。 |
| 索引建设 | T78 | ⏳ 待执行 | 为正式成果层 / 治理层 / 线程层 / 总索引层建立显式映射清单。 |

## T73（重构版）: 统一现场判定协议的首轮落地
...
## T75（重构版）: 完成态分级 + 成果分层 + 回滚口径 + 对外话术统一收口
