...
## 十一、附录：Write 防护规则（已实施）

**问题**: 会话 14 中一次写入操作缺少必要内容，导致连续多次写入失败（aborted）形成死循环。

**已实施的修复**（写入 rules.md "文档写入规范"章节）:
- 写入前必须确认写入内容非空（不要提交空 payload）
- 连续失败 2 次 → 立即停止重试，改为一次性重写该文件（Write/替换策略）
- 禁止对同一文件连续调用 3 次以上失败的写入操作
- 失败后的恢复策略：Read → 确认断点 → Edit/Write 分块补齐（必要时用 Bash 追加，避免一次性超大写入）

**未采用 preToolUse Hook 的原因**: 会话 8 讨论过，工具调用前 Hook 每次写操作都会触发；一次对话 5-20 次时额外消耗显著，成本不划算。规则约束是零成本方案。
