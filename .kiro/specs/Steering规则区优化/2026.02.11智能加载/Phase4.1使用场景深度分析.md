...
```
## 文档写入规范
- 新建文件：用 `Write`，单次写入建议不超过 150 行
- 已存在文件追加：优先 `Edit`；大段追加用 `Bash` 分块（`cat >> 绝对路径 << 'EOF' ... EOF`）
- 超过 200 行的内容必须分段写入
- 不要在表格或代码块中间断开
- 如果写入被中断（aborted），从断点继续分块追加（优先 Edit 定位插入点，必要时 Bash 追加）
```
