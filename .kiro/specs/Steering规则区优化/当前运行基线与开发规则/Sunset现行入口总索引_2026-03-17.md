...
- 如果当前会话 skill inventory 没有显式列出 `sunset-startup-guard`：
  - 先按 discovery gap 理解
  - 继续手工执行同级启动核查
  - 并在 `C:\Users\aTo\.codex\memories\skill-trigger-log.md` 留下 `manual-equivalent`
- 工作区路由 skill：
...
## 8. 一句话口径
- 现在真正的执行入口应先看 `AGENTS.md + Sunset当前规范快照_2026-03-22.md`；`03-17` 的唯一状态说明保留历史背景，`.kiro/steering` 只补通用结构与领域细则。
