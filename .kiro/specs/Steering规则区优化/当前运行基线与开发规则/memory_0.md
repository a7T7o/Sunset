...
## 2026-03-16 补记：`agent-a2df3da0` 已降级为历史本地噪音，不再作为现行治理对象
- 当前子工作区主线仍是“共享状态层收口后，再决定 Git 规则是否需要分层放宽”；本轮插入式子任务是修正两处仍会误导后续线程的旧口径。
- 本轮已确认：
  - `D:\Unity\Unity_learning\Sunset\.kiro\steering\git-safety-baseline.md` 仍把 `agent-a2df3da0` 当成具体治理对象；
  - `D:\Unity\Unity_learning\Sunset\AGENTS.md` 仍残留“长期功能线程默认进入独立 worktree”的旧说法。
- 本轮已修正：
  - 把 `git-safety-baseline.md` 中的 `agent-a2df3da0` 具体案例改写为“`.claude/worktrees/` 下的历史本地工作树噪音”通用口径，明确：若某个 agent 目录已被手动删除或已确认只是临时残留，就不再围绕它单独建规则；
  - 把项目根 `AGENTS.md` 收紧到与现行活文档一致：长期功能线程默认先从主项目根目录进入，再按真实任务切 `codex/` 分支，`worktree` 仅保留为高风险隔离、故障修复和特殊实验。
- 本轮稳定结论：
  - `agent-a2df3da0` 现在应被视为已经失效的历史噪音，不应再进入现行调度、锁定、验收或线程入口判断；
...
## 2026-03-16 补记：A 类热文件写锁转交新增“文件级固化/停车”闸门
- 当前子工作区主线已从“首批复工是否通过”推进到“第二批发锁前如何防止 shared worktree 同文件混线”；本轮用户要求我直接吸收这条治理补丁，而不是继续把它写成外发建议。
- 本轮已正式采纳的准确规则：
  - 风险判断成立：如果上一线程在 shared worktree 里留下同一个 A 类热文件的 dirty，下一线程再继续写，会破坏白名单提交与归属判断；
  - 但不能粗暴扩写成“发下一把锁前整个工作树必须 clean”，因为 Sunset 当前仍允许仓库中存在其他 remaining dirty，只要它们不混入本轮白名单同步；
  - 新规则的判断单位改为“文件”：同一个 A 类热文件写锁转交前，上一 owner 在该文件上的改动必须进入 `Committed`、`Parked`、`Abandoned` 三者之一，明确禁止 `Mixed-in-place`。
- 本轮已同步修订：
  - `Sunset当前唯一状态说明_2026-03-13.md`：补入 A 类热文件写锁转交的硬规则；
  - `Sunset文件认领与锁定表_2026-03-15.md`：补入 2026-03-16 锁转交补充口径，并明确 `farm` 当前仍只允许零写入只读复工。
- 当前恢复点：
  - `farm` 的只读复工继续有效；
  - 后续若 `farm` 申请 `Primary.unity` 的写锁，必须先检查该文件在 `spring-day1` 上一轮改动是否已经文件级固化/停车，而不能只看“锁已释放”。

## 2026-03-16 补记：本轮治理文档已更新，但 Git 同步因 governance 白名单过宽而暂缓
- 当前子工作区主线仍是治理执法收口；本轮在文档与记忆更新后，已按规则执行 `git-safe-sync.ps1 -Action preflight -Mode governance` 做同步前检查。
- preflight 结果虽显示 `CanContinue=True`，但 `Allowed` 集合除了本轮 5 个目标文件外，还自动卷入了 `2026.03.16冻结文档汇总/` 下多份无关未提交治理文档；如果直接 `sync`，会形成一次混合治理提交。
- 因此本轮决定：先保留这次口径修正于本地，不执行 `sync`；后续如要提交，需先单独收口那批冻结治理文档，或收紧 `git-safe-sync.ps1` 的 governance 白名单边界。

## 2026-03-16 补记：`farm` 只读复工通过，第三轮裁决转向项目级 Missing script
- 当前子工作区主线已进入第三轮排期；本轮先接收 `farm` 只读复工报告，再独立复核锁池与 Console。
- 本轮已确认：
  - `farm` 当前无需任何 A 类写锁，第二批只读复工可判定为通过；
  - `.kiro/locks/active/` 当前只有 `.gitkeep`，说明所有活动写锁均已归还；
  - Console 当前仍能读到 8 条 `The referenced script (Unknown) on this Behaviour is missing!`，这不是 `farm` 专属问题。
- 本轮已进一步定位：
  - 缺脚本不是 `fileID: 0` 级别，而是场景 / Prefab 中仍引用了一批已经不存在的脚本 guid；
  - `Primary.unity` 中的缺链对象集中在 `EventSystem`、`Main Camera`、`CinemachineCamera`、`UI`、`DialogueCanvas`、`Background`、`头像`、`SpeakerNameText`、`DialogueText`、`ContinueButton`、`Icon`；
  - `DialogueValidation.unity` 和一批 `Assets/222_Prefabs/UI/...` Prefab 也存在同类缺链，说明这是“项目级 UI / 场景序列化残留”，不是 `farm` 新引入的问题。
- 当前治理裁决：
  - `farm` 无需补发 A 类写锁；
  - `NPC`、`导航检查`、`遮挡检查` 继续冻结 A 类写操作；
  - 下一优先事项不再是给 `farm` 或 `NPC` 发锁，而是认领项目级 Missing script 清理；
  - 由于 `Primary.unity` 仍处于 `spring-day1` 上一轮 dirty 语境下，若要让“新 owner”写 `Primary.unity`，必须先满足文件级固化/停车闸门；若仍由同 owner 延续处理，则不构成跨 owner 转交。

## 2026-03-16 补记：`spring-day1` Missing script 复验通过，项目级红错误已降为 0
- 当前子工作区主线继续是第三轮验收收口；本轮在接收 `spring-day1` 回执后，我独立执行了最小复验：`Check-Lock.ps1` 复核 `Primary.unity` 已释放、清空 Console、进入一次 PlayMode、退出后再读 Console。
- 本轮独立证据成立：
  - `Primary` 仍是活动场景，且 `isDirty=false`（Unity 场景保存态正常）；
  - `Primary.unity` 的活动锁当前为 `unlocked`，并新增释放记录 `A__Assets__000_Scenes__Primary.unity.lock.20260316-164140.released.json`；
  - 清空 Console 后重新跑 Play/Stop，`The referenced script (Unknown) on this Behaviour is missing!` 为 `0` 条；
  - 当前 Console 只剩常规初始化日志与 `There are no audio listeners in the scene` warning。
- 当前治理结论更新为：
  - `spring-day1` 针对 Missing script 的这轮 checkpoint 验收通过；
  - 当前项目级红错误已从“8 条 Missing script”下降到“0 条 Missing script”；
  - 但项目仍未进入完全自由并行写入态，因为 `Assets/000_Scenes/Primary.unity` 与 `Assets/YYY_Scripts/Story/UI/DialogueUI.cs` 仍是 shared worktree 中未固化的 hot-file dirty。
- 当前恢复点：
  - 下一步不再发新的 A 类写锁；
  - 先处理这两个 hot file 的文件级固化/停车（`Committed` / `Parked` / `Abandoned`），再决定是否开放下一轮跨 owner 写入。

## 2026-03-16 补记：治理线程完成深度自我复盘，L5 唯一缺口收缩为 hot-file 文件级固化
- 当前子工作区主线已从“第三轮验收收口”进一步收紧为“治理终局收口”：不再继续扩写制度，也不再把功能验证和工具链问题混成主线，而是直接判断项目距离 `L5` 还差哪一个物理动作。
- 本轮实际回读了：
  ...
  - 当前线程 `memory_0.md` 与父层 `memory.md`
- 本轮再次独立核验当前现场：
  - `unityMCP` 成功读取活动场景 `Primary`，`isDirty=false`；
  - Unity Console 当前仅剩初始化日志与 `There are no audio listeners in the scene` warning，没有新的项目级错误；
  - `.kiro/locks/active/` 当前只有 `.gitkeep`，活动锁池为空；
  - Git 现场中 `Assets/000_Scenes/Primary.unity` 与 `Assets/YYY_Scripts/Story/UI/DialogueUI.cs` 仍是 shared worktree 的 tracked dirty，`git diff --stat` 仍显示两者存在大体量未固化改动。
- 本轮对治理线程自身的最终自审结论：
  - 正确方向已经成立的部分：活文档入口统一、旧 worktree 默认口径退役、`unityMCP` 统一、共享状态层建立、物理锁落地、`Committed/Parked/Abandoned` 文件级转交闸门建立；
  - 这条线程前面的主要偏航包括：过度把“写规则/写模板”当成治理完成、曾把历史 worktree 路径长期当成默认入口、曾把“锁释放”误当成“文件可直接转交”、以及低估了 `git-safe-sync.ps1` 在 dirty shared worktree 下对 `ensure-branch` 的真实限制。
- 当前 L1-L5 坐标已收紧为：
  - `L1`：通过，现行入口、线程入口、锁表、验收模型已经形成统一口径；
  - `L3`：通过，当前没有新的项目级红错误，Console 阻断已清零；
  - `L4`：通过，`unityMCP` 能稳定读取 `Primary` 与 Console，关键 live 场景链路已可核验；
  - `L2`：未通过，原因不是“仓库必须全 clean”，而是 `Primary.unity` / `DialogueUI.cs` 这两个 A 类 hot file 仍未进入 `Committed`、`Parked` 或 `Abandoned`，因此不满足跨 owner 写入前的文件级归属要求；
  - `L5`：未通过；当前唯一剩余准入缺口就是上述 hot-file 文件级固化/停车。
- 当前恢复点：
  - 不再给任何新线程发这两个 hot file 的写锁；
  - 终局动作不应再写成“继续修代码”，而应直接转向 `spring-day1` 上一轮 hot-file 改动的文件级 `Committed` 或 `Parked`；
  - 若继续尝试在当前 shared dirty worktree 中直接 `ensure-branch`，会被 `git-safe-sync.ps1` 拦下，因此后续必须先设计可执行的固化方案，再谈正式跨入 `L5`。

## 2026-03-16 补记：已审核 `spring-day1` hot-file 固化回执，需纠正 `Parked` 定性
- 当前子工作区主线仍是治理终局收口；本轮接收并审核了 `spring-day1` 的 hot-file 固化回执，重点核了任务分支、commit、提交文件集，以及 shared worktree 中两份 hot file 的真实状态。
- 本轮已独立核实：
  - 分支 `codex/spring-day1-hotfile-solidify-20260316` 真实存在；
  - commit `9b9a6bd0dd7c5ee7d18cc82e3ea9da74a146bf9d` 真实存在，且当前由该任务分支持有；
  - 该 commit 的文件集仅包含 `Assets/000_Scenes/Primary.unity` 与 `Assets/YYY_Scripts/Story/UI/DialogueUI.cs`；
  - shared worktree 里的这两个文件当前仍是 `M`，但把当前 worktree 内容直接与该 commit 比较时已无内容差异，说明 shared worktree 现在保留的正是已固化版本。
- 本轮纠偏结论：
  - 按 `Sunset文件认领与锁定表_2026-03-15.md` 的现行定义，这一状态不应写成 `Parked`；
  - 原因是 `Parked` 要求“已导出为 patch/快照等可回放证据，且 shared worktree 中该文件已恢复到可安全交接状态”，而当前 shared worktree 中这两个文件仍是 tracked dirty，尚不满足“已恢复到可安全交接状态”；
  - 更准确的定性应是：这两份 hot file 已经 `Committed` 到独立任务分支，但 shared worktree 仍未 clean，因此暂不允许把同文件写锁直接交给下一 owner。
- 当前恢复点：
  - 后续治理口径应改为“Committed but not yet handoff-safe”，不要再把这类状态写成 `Parked`；
  - 下一步若要开放下一 owner 写入，仍需先处理 shared worktree 的对齐/清理，而不是重新质疑这次固化是否成立。

## 2026-03-16 补记：当前已可恢复大部分正常开发，但尚未达到完全自由并行写入
- 当前子工作区主线已从“审核固化回执”推进到“给出是否可全面开发的最终定性”。
- 当前准确状态应写成：
  - 项目级阻断已解除，`L1/L3/L4` 当前可视为通过；
  - 大部分正常开发已经可以继续，前提是继续遵守 `main + governance` / `codex/... + task` 的现行 Git 规则；
  - 但 `L2/L5` 仍未完全通过，因为 `Primary.unity` 与 `DialogueUI.cs` 虽已 `Committed` 到独立任务分支，却仍留在 shared worktree 中处于 tracked dirty，尚未成为可安全转交的 clean 基线。
- 因此当前最准确的对外口径是：
  - 可以恢复正常开发；
  - 但还不能宣告“完全自由并行开发”；
  - 任何线程暂时都不应跨 owner 继续写 `Primary.unity` 与 `DialogueUI.cs`。
- 当前唯一下一步：
  - 处理 shared worktree 中这两个 hot file 的对齐/清理，使其从“Committed but not handoff-safe”进入真正可交接状态；
  - 在此之前，其他与这两个文件无关的开发可以继续推进。

## 2026-03-16 补记：hot-file 对齐已完成，当前已进入可全面继续开发状态
- 当前子工作区主线已从“判断是否可全面开发”推进到“完成最后对齐并正式收口”。
- 本轮由当前治理线程直接执行并核实：
  - `main` 从 `c0222395` 快进到 `9b9a6bd0dd7c5ee7d18cc82e3ea9da74a146bf9d`；
  - `origin/main` 也已推送到同一提交；
  - `Assets/000_Scenes/Primary.unity` 与 `Assets/YYY_Scripts/Story/UI/DialogueUI.cs` 当前在 shared `main` 中已 clean；
  - `.kiro/locks/active/` 当前为空，仅剩 `.gitkeep`；
  - Unity Console 当前仍仅剩初始化日志与 `There are no audio listeners in the scene` warning，没有新的项目级阻断。
- 当前最终治理结论更新为：
  - 此前唯一剩余的 hot-file handoff 缺口已关闭；
  - `L1/L2/L3/L4` 当前均可视为通过；
  - `L5` 现在可以判定为通过，项目已回到“可全面继续开发”的常态，但仍继续遵守现行 Git 规则与文件认领规则。
- 当前恢复点：
  - 后续线程可以恢复正常开发；
  - `Primary.unity` 与 `DialogueUI.cs` 不再因上一轮 shared dirty 被额外冻结；
  - 之后若再次涉及 A 类热文件并发写入，仍按锁机制与 `Committed/Parked/Abandoned` 规则执行。

## 2026-03-16 补记：已新增 L5 开工总览，并把当前唯一状态说明同步到收口后现实
- 当前子工作区主线已从“最后 hot-file 对齐”转入“L5 后正常开发入口梳理”。
- 本轮已新增干净活文档：
...
- 当前新增口径已固定：
  - 项目已达 `L5`，可全面继续开发；
  - 但“全面继续开发”仍然遵守 `main + governance` / `codex/... + task` / A 类热文件锁机制；
  - 当前活文档层中存在一批文件本体错码，不能再当成一线入口继续叠改，后续应优先重写或替换；
  - 新增的 `Sunset L5开工清单与治理续办总览_2026-03-16.md` 当前承担“版本快照 + 开工清单 + 线程唤醒 + 治理续办”的干净入口。
- 当前恢复点：
  - 下一步若继续治理，优先做活文档错码重写与 skills 错码修复；
  - 若切回业务推进，则直接按新总览文档中的线程优先级唤醒各线程开工。

## 2026-03-16 补记：已纠正现行状态入口命名，并挂接新的治理续办代办工作区
- 当前子工作区主线已从“L5 后正常开发入口梳理”推进到“现行入口命名纠偏与代办总入口挂接”。
- 本轮已完成的实际动作：
  - 新建 `D:\Unity\Unity_learning\Sunset\.kiro\specs\Steering规则区优化\当前运行基线与开发规则\Sunset当前唯一状态说明_2026-03-16.md`，作为新的现行唯一状态入口；
  - 将 `Sunset当前唯一状态说明_2026-03-13.md` 降级为历史锚点 / 路由页，不再继续承载“当前状态”身份；
  - 更新 `文档重组总索引_2026-03-13.md`，补入新的当前状态入口、`000_代办/codex` 代办入口，以及“历史命名 / 现行命名”的判断标准；
  - 更新 `Sunset L5开工清单与治理续办总览_2026-03-16.md`，把治理入口链接切到新的 `2026-03-16` 状态文件，并显式登记 `000_代办/codex`。
- 本轮新增的稳定判断：
  - “持续描述当前状态 / 当前规则 / 当前默认入口”的文件，不应继续沿用已经过时的日期命名；
  - “描述某次重组、归档、治理阶段或事件”的文件，可以保留事件日期，但必须明确其身份是索引、底册、阶段记录或历史锚点，而不是现行入口；
  - L5 之后的治理续办，不再回灌到旧的全局 `tasks.md`，而是由 `D:\Unity\Unity_learning\Sunset\.kiro\specs\000_代办\codex` 统一承接。
- 本轮 Git 侧独立核实：
  - `git rev-list --left-right --count origin/main...main` 返回 `0 0`，说明 `main` 与 `origin/main` 同步；
  - 当前真实所在分支是 `codex/npc-asset-solidify-001`，不是 `main`；
  - `git status --short` 当前约为 `72` 条工作树状态项，因此 VSCode 里看到的 `113 未上传` 不能被直接解释为“当前主线还有 113 个未推送提交”，更像是 IDE 对工作树脏改、其他分支或缓存统计的混合显示。
- 当前恢复点：
  - 继续治理时，优先推进 `000_代办/codex/01_现行入口与活文档重构`、`02_skills_AGENTS_与执行机制重构`、`05_四件套与代办体系重构`；
  - 若转回业务推进，则用户可按 L5 总览里的线程优先级继续解冻线程。

## 2026-03-16 补记：skills 执行层继续压实，已新增锁治理与编码审计 skills
- 当前子工作区主线仍是 L5 后治理续办，不是业务开发；本轮继续把“规则必须靠 skill 落地”这件事做实。
- 本轮已完成：
  - 重建 `C:\Users\aTo\.codex\skills\sunset-thread-wakeup-coordinator\agents\openai.yaml`，修复此前 ANSI/GBK 编码漂移，并补入 `allow_implicit_invocation`；
  - 新增 `sunset-lock-steward`，把 `.kiro/locks/`、A 类热文件、`Committed/Parked/Abandoned` 交接闸门与 `Acquire-Lock.ps1` / `Release-Lock.ps1` 的使用流程收成统一 skill；
  - 新增 `sunset-doc-encoding-auditor`，把“先区分终端假乱码与真实编码损坏，再决定转 UTF-8、重建或忽略历史文件”的流程收成统一 skill；
  - 更新项目 `AGENTS.md`，把锁治理与编码审计写成新的优先技能入口；
  - 更新 03-16 状态入口、L5 总览、活跃线程总表与阶段 tasks，使现行文档不再笼统误报“部分治理型 skills 的 SKILL.md 错码”。
- 本轮稳定结论：
  - 当前真正高频且值得工具化的治理动作，已经至少形成三条健康链路：线程唤醒、锁裁决、编码审计；
  - 编码问题必须先做“真坏 / 假坏”区分，不能再因为 PowerShell 默认读法错了就误判整批文档损坏；
  - 锁机制也不能只靠总表口头执行，必须让 skill 与 `.kiro/scripts/locks/` 脚本协同工作。
- 当前恢复点：
  - 后续若继续治理，优先补 `sunset-release-snapshot` 与历史首版文档退役；
  - 若切回业务推进，则现有项目级 skills 已可作为真实入口约束线程行为。

## 2026-03-16 补记：已补齐 `sunset-release-snapshot`，治理 skill 形成四件套
- 当前子工作区主线仍是治理续办；本轮继续把“快照不该再靠手工拼接”这件事落实成 skill。
- 本轮已完成：
  - 新增 `sunset-release-snapshot`；
  - 更新项目 `AGENTS.md`、全局 skill 注册表、03-16 状态入口、L5 总览、活跃线程总表与阶段 tasks；
  - 将现行治理 skill 集合明确为四条：线程唤醒、锁裁决、编码审计、阶段快照。
- 本轮稳定结论：
  - 现在最核心的治理高频动作已经不再停留在“文档规范”，而是都拥有了项目级 skill 入口；
  - 版本快照应被看作阶段证据与归档入口，不应继续伪装成持续变化的现行状态文档。
- 当前恢复点：
  - 后续若继续治理，优先转向历史首版文档退役、旧 `tasks.md` 兼容件清理、以及是否需要更强的技能强制约束；
  - 若切回业务推进，则这四个 skill 已可直接协助新一轮线程开工和阶段收口。

## 2026-03-16 补记：冻结汇总已归档，L5 阶段快照已落盘
- 当前子工作区主线仍是治理续办；本轮继续把“冻结样本”和“版本快照”从聊天态转成正式归档证据。
- 本轮已完成：
  - 将 `2026.03.16冻结文档汇总` 迁入 `文档归档\2026-03-Codex恢复与迁移收口\06_L5解冻与阶段快照_2026-03-16`；
  - 新增 `Sunset L5稳定基线快照_2026-03-16.md`，记录当前稳定基线、实际工作树分支、锁池、活动场景、Console 和线程结论；
  - 将 `04_冻结文档归档与版本快照/tasks.md` 全部勾完；
  - 纠正 `Sunset当前唯一状态说明_2026-03-16.md` 与 `Sunset L5开工清单与治理续办总览_2026-03-16.md` 中已经过时的 Console warning 口径。
- 本轮稳定结论：
  - `2026.03.16冻结文档汇总` 已从活跃层降级为阶段证据，不再应继续悬挂在当前治理根层；
  - 当前阶段快照显示：稳定基线仍是 `main@9b9a6bd0...`，但实际工作树已处于 `codex/farm-1.0.0-1.0.1`，这类差异必须始终显式写清；
  - `Assets/Screenshots/farm_checkpoint_primary_maincamera.png` 当前应视为 `farm` 验收证据，不属于治理默认基线。
- 当前恢复点：
  - 后续若继续治理，优先退役 03-15 历史首版和旧 `tasks.md` 兼容件；
  - 若切回业务推进，当前快照已经足够给后续线程和治理线程做共同锚点。

## 2026-03-16 补记：现行共享表的 live 锚点已纠偏到 `codex/farm-1.0.0-1.0.1`
- 当前子工作区主线仍是治理续办；本轮补的是“现行共享表仍挂着旧 live 快照”的残差。
- 本轮已完成：
  - 将 `Sunset当前工作树dirty与WIP归属表_2026-03-16.md` 的当前分支 / `HEAD` 从旧的 NPC 快照改为当前真实工作树 `codex/farm-1.0.0-1.0.1 @ 9b9a6bd0...`；
  - 将 `Sunset活跃线程总表_2026-03-16.md` 同步改到当前真实 live 锚点；
  - 将 `NPC` 行改写成“最近一次 NPC 快照”口径，避免误把线程历史分支写成当前现场。
- 本轮稳定结论：
  - 现行共享表与阶段快照必须对齐同一份当前 Git 证据；
  - 历史线程分支信息可以保留，但必须标清它是线程快照，不是当前 live 工作树。
- 当前恢复点：
  - 后续若继续治理，优先处理旧首版与旧 `tasks.md` 兼容件；
  - 若切回业务推进，当前 live 表与阶段快照已经对齐到同一现场。

## 2026-03-16 补记：`03-13` 状态正文与 `03-15` 首版已退出活目录，旧 `tasks.md` 已缩退为兼容路由页
- 当前子工作区主线仍是 L5 后治理续办；本轮实际收的是“活目录看上去还是旧新混放”的入口残差。
- 本轮已完成：
  - 将 `Sunset当前唯一状态说明_2026-03-13.md` 正文迁入 `06_L5解冻与阶段快照_2026-03-16/01_03-13历史状态锚点退役_2026-03-16/`；
  - 将 `03-15` 首版 `Git` / `多线程分析` / `线程总表` / `锁表` / `dirty 表` / `验收模型` / `汇总模板` 迁入 `06_L5解冻与阶段快照_2026-03-16/02_03-15首版共享表与模型退役_2026-03-16/`；
  - 将旧全局 `tasks.md` 重写为只负责跳转到 `000_代办/codex` 的兼容路由页；
  - 重写 `文档重组总索引_2026-03-13.md`，补齐退役映射、新归档位置、四个治理 skills 与当前代办入口；
  - 更新 `Sunset当前唯一状态说明_2026-03-16.md`、`Sunset L5开工清单与治理续办总览_2026-03-16.md`、`Sunset Git系统现行规则与场景示例_2026-03-16.md`、`基础规则与执行口径.md` 与 `Codex迁移与规划/README_迁移结束与路由说明_2026-03-14.md` 的路由口径。
- 本轮稳定结论：
  - 用户指出得对：旧日期的“当前状态正文”不该继续和现行入口并列悬挂；
  - `文档重组总索引_2026-03-13.md` 可以保留 `03-13` 命名，因为它记录的是重组工程事件本身，而不是“当前唯一状态”；
  - 四件套治理续办已在 `05_四件套与代办体系重构/` 形成 `tasks.md + memory.md` 的干净阶段结构。
- 当前恢复点：
  - 后续继续治理时，优先转向旧超长 `memory` 分卷与剩余历史错码对象巡检；
  - 若切回业务推进，当前活目录已经足够清爽，不再需要从旧首版文件里猜现行入口。
