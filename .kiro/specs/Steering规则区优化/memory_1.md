...
**用户需求**:
> 希望 AI 能智能判断输入内容，自动用 Read 加载相关 steering 规则，替代手动 #rulename
...
**中断原因**: Bash 追加漏掉 EOF 分隔或内容参数为空导致连续 aborted 死循环，用户手动 Cancel

**用户反馈**: 希望通过规则约束或 Hook 解决“写入追加内容为空/分隔错误”导致的死循环问题
...
- rules.md 增加 Write/Edit/Bash 写入防护规则
- 详见子工作区 memory 会话 15
