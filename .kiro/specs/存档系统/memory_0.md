...
## 2026-04-18 只读审计：跨场景 world-state 连续性对象合同盘点

### 用户目标

- 只读排查 Sunset 当前“跨场景 / 跨天 world-state 连续性”是否闭环。
- 重点限定在：
  ...
  - `CropController`
  - `DoorTrigger`
  - 必要时补读 `SaveManager` 与 `InventoryService`

### 本轮完成

1. 只读核对 `SaveManager -> PersistentObjectRegistry -> PersistentPlayerSceneBridge -> DynamicObjectFactory` 的当前主链，确认正式存档与离场 scene runtime snapshot 目前如何分工。
2. 逐个核对树木、石头、箱子、掉落物、农地、作物的 `ShouldSave / Save / Load / SetPersistentId / ApplyOffSceneElapsedDays`。
3. 补读 `DoorTrigger.QueueSceneEntry(...)`，确认普通门切场时确实会触发 bridge 离场快照。
4. 额外钉死两个高风险掉落物断点：
   - `TreeController.SpawnDrops()/SpawnStumpDrops()` 没有给掉落物写 `sourceNodeGuid`
   - `WorldItemPickup.Reset()` 不清 `sourceNodeGuid`，而 `ItemDropHelper` 复用对象池生成玩家丢弃物时也不会主动清空

### 本轮关键判断

1. 现在已经存在一条真正可用的“跨场景 world-state snapshot 合同”：
   - `PersistentPlayerSceneBridge.CaptureSceneWorldRuntimeState()`
   - `RestoreSceneWorldRuntimeState()`
   - `ExportOffSceneWorldSnapshotsForSaveInternal()`
   - `ImportOffSceneWorldSnapshotsFromSaveInternal()`
   负责把离场 scene 的 `FarmTileManager / Crop / Chest / Drop / Tree / Stone` 缓存在 bridge，并在正式 slot-save 时把“当前未激活的 scene”一起入盘。
2. 当前真正有“跨天 catch-up 消费合同”的只有：
   - `TreeController.ApplyOffSceneElapsedDays(...)`
   - `CropController.ApplyOffSceneElapsedDays(...)`
   - `FarmTileManager.ApplyOffSceneElapsedDays(...)`
   其中 tree 是即时消费，crop/farm 是 bridge 在恢复后延迟一拍统一补算。
3. 农地/作物是这批对象里合同最完整的一组：
   - manager 负责空地寿命与水分残留
   - crop 负责成长 / 缺水 / 过熟 / 过季
   - bridge 还专门用 `FarmTileManager -> Crop` 的恢复优先级，加上 deferred catch-up，避免 manager 先把“本来有 crop 的 tile”误判为空地。
4. 掉落物是当前最危险的断点：
   - 石头掉落会写 `sourceNodeGuid`
   - 树木掉落不会写
   - 对象池回收时 `WorldItemPickup.Reset()` 又不清这个字段
   - 于是 bridge/save/load 侧用于“来源节点还活着就别重建掉落物”的去重保护，可能一边漏掉树木掉落重复，一边又把玩家手动丢出的物品误判成旧资源掉落。

### 结论分层

- 已有跨场景合同：
  - 树 / 石 / 箱 / 掉落物 / 农地 / 作物 的离场 snapshot、回场绑定、缺对象重建、反向修剪。
- 已有跨天合同：
  - 树 / 农地 / 作物。
- 仍缺或不完整：
  - 树：off-scene catch-up 只补成长，不补天气/冬季事件语义。
  - 石：只有存在/血量/阶段连续性，没有任何跨天语义。
  - 箱：只有内容与锁状态连续性，没有跨天语义，也不保存 UI/open runtime。
  - 掉落物：存在/重建合同有，但 `sourceNodeGuid` 生产与清理不一致。
  - DTO 层：`GameSaveData.farmTiles` 仍是历史残留，现行 farm 合同实际走 `worldObjects + genericData`。
...
- `D:\Unity\Unity_learning\Sunset\Assets\YYY_Scripts\World\WorldItemPool.cs`
...
- 已完成：静态代码审计与旧 memory 交叉核对。
- 未完成：Unity live 往返复现、跨天实机矩阵、打包态终验。
- 当前结论级别：`静态推断成立`，不是 live 终验。

### 恢复点

- 如果后续进入真实施工，打包前最小且最安全的一刀应优先盯住掉落物合同：
  1. 树木掉落补 `sourceNodeGuid`
  2. 对象池复用时清理旧 `sourceNodeGuid`
  3. 再看树木 off-scene catch-up 是否需要补冬季 / 天气语义

## 2026-04-23｜保本上传运行态记录

### 结论
1. docs/prompt own 簇已成功归仓并 push：
   - 提交 SHA：`90e95fc7`
   - push 状态：`origin/main` 已对齐到该提交
2. `Data/Core` 代码簇未归仓：
   - `InventoryItem.cs`
   - `SaveDataDTOs.cs`
   - `SaveManager.cs`
...
### 直接 blocker
1. `Ready-To-Sync.ps1` 对上述 3 文件做 preflight 时，父进程会拉起：
   - `sunset-git-safe-sync.ps1 -Action preflight`
   - 临时 canonical `git-safe-sync.ps1`
   - `dotnet CodexCodeGuard.dll --phase pre-sync`
   - `git diff --name-status HEAD --`
2. 当前卡死点实际落在：
   - `CodexCodeGuard -> git diff --name-status HEAD --`
3. shell timeout 会提前杀掉父级 `Ready-To-Sync`，导致：
   - 状态文件仍停留 `ACTIVE`
   - `ready-to-sync.lock` 变成 stale runtime file
   - 子级 preflight / codeguard / git 进程残留

### 本轮止血
1. 已确认 stale `ready-to-sync.lock` 在无持有者时可安全读取并删除。
2. 已清理本线程留下的挂起进程：
   - `Ready-To-Sync`
   ...
   - 下游 `git diff`
3. 已执行：
   - `Park-Slice -ThreadName '存档系统' -Reason 'park-after-codeguard-preflight-hang'`

### 恢复点
1. 下次如果要继续收这 3 个 `Data/Core` 文件：
   - 先确认 `ready-to-sync.lock` 不存在且无持有者
   - 再查 `CodexCodeGuard` 为何在 `git diff --name-status HEAD --` 这一步长时间不返回
2. 当前不要再把这 3 个文件误判为 shared/mixed；这轮真实 blocker 是运行态 preflight。

## 2026-04-23｜历史小批次上传尝试 02
...
### 本轮结论
1. 这 3 个文件可以视作同一真实历史小批：
   - `InventoryItem.RestoreInstanceIdForLoad()` 被 `SaveDataDTOs.FromSaveData()` 直接调用
   - `SaveDataDTOs` 与 `SaveManager` 在默认存档、load-restore、off-scene snapshot DTO 上直接相连
2. 这组小批没有上传成功。
3. 第一真实 blocker 仍然是 `CodeGuard`：
   - `Ready-To-Sync preflight hangs at CodexCodeGuard -> git diff --name-status HEAD --`

### 本轮操作
1. 已执行 `Begin-Slice`
   - `historical-mini-batch-data-core-20260423`
2. 已对白名单这 3 个文件做一次真实 `Ready-To-Sync` 尝试
3. 观察窗内未返回后，已安全终止本线程自己拉起的挂住进程链
4. 已执行 `Park-Slice`
...
### 恢复点
1. 先排 `CodeGuard` 对这 3 个 `Data/Core` 文件的 preflight 挂死。
2. 后续只重试这同一组历史小批，不换第二批。

## 2026-04-24｜DataCore 预同步工具 incident 只读定性

### 结论
1. 这轮没有重跑复现；现有证据已足够。
2. incident 不再按“业务上传失败”记录，而按“工具 preflight 挂死”记录。
3. 当前最靠近根因的层是 `CodexCodeGuard Program.cs`，不是 `Ready-To-Sync` 外壳。
...
1. 上轮已真实观测到完整进程链：
   ...
   - `sunset-git-safe-sync preflight`
   - 临时 canonical `git-safe-sync.ps1`
   - `dotnet CodexCodeGuard.dll`
   - `git diff --name-status HEAD --`
2. 当前源码补证：
   - `Ready-To-Sync.ps1` 只是同步执行 `Invoke-StableGitSafeSync -Action preflight`
   - `StateCommon.ps1` 的 `Invoke-StableGitSafeSync` 只是再启动稳定 launcher 并等待返回
   - `git-safe-sync.ps1` 的 `Invoke-CodeGuard` 只是同步等待 `dotnet CodexCodeGuard.dll` 的 JSON
   - `Program.cs` 内部的 `GitDirtyState.Load()` 会跑整仓 `git diff --name-status HEAD --`
   - `Program.cs` 的 `RunProcess()` 采用 `ReadToEnd(stdout)` 后再 `ReadToEnd(stderr)` 的串行读取
3. 所以当前最合理解释是：
   - `CodexCodeGuard` 内部 process/pipe 级卡死，导致 JSON 永远吐不出来
   - 上游 `git-safe-sync / Ready-To-Sync` 只是被动等待

### 当前裁定
- 这件事应升级给治理/工具位，不建议继续交业务线程自己排。

## 2026-04-25｜工具修复后最小复核结果
...
2. 本轮没有再出现：
   ...
   - `no JSON`
   - `bad JSON`
3. 当前这组已降级成真实业务 blocker。

### 真实 blocker
1. `Ready-To-Sync` 稳定返回代码闸门失败。
2. 第一真实 blocker = `Assets/YYY_Scripts/Data/Core/SaveManager.cs` 上 `4` 条 `CS1061`：
...
### 本轮边界
1. 没有修业务代码。
2. 没有扩到 `StoryProgressPersistenceService.cs`。
3. 没有扩到任意 save/story/workbench tests。
4. 没有继续第二批。

### 当前恢复点
1. 这条线后续应回到业务线程修 `SaveManager.cs` 的 4 条 `CS1061`。
2. 不需要继续把这件事当工具 incident 追。
