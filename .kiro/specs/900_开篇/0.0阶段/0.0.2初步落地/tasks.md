...
## 补充：NPC 最小接轨（并入当前主线，不切换主任务）
- [ ] 10. 当前交互 NPC 的最小对话占用
  - 对话开始时冻结当前交互 NPC 的漫游 / 移动
  - 对话结束时恢复该 NPC 的漫游
  - 允许在开聊瞬间让该 NPC 面向玩家
  - 只处理“当前这一个交互 NPC”，不扩成全图 NPC 调度
  - 验收：`NPC001` 在对话期间不再继续自主乱走，结束后能恢复原先漫游
- [ ] 11. NPC 接轨边界保持最小化
  - 不在本 checkpoint 内扩写：群体 NPC 广播暂停、剧情演出位 / 站位点、导航核心礼让 / 会车 / 动态避让
  - 这些内容保留到后续与 NPC / 导航线程的正式协作
