...
**内容**：NPC 自动移动与环境社交规划起稿

**用户需求**：
先将 NPC 自动移动的基础设想完整写入新的 `npc规划001.md`，要求结合现有导航系统理解后再做第一轮拓展，作为后续脑洞风暴基线。

**完成任务**：
1. 回读当前导航规范、导航工作区记忆与导航相关代码，确认现有 `NavGrid2D`、`PlayerAutoNavigator`、`INavigationUnit` 可作为 NPC 自动移动的适配基础。
2. 回读对话数据结构，确认 `DialogueNode` 已存在 `isBubble` 字段，适合作为后续 NPC 气泡对话的潜在接点。
3. 新建 `npc规划001.md`，完整记录用户提出的“随机移动 + 短停 + 长停 + 自言自语 / 聊天冒泡”设想，并补出第一轮系统拆分、V1 边界和后续脑暴问题。

**修改文件**：
- `.kiro/specs/001_BeFore_26.1.21/导航系统重构/npc规划001.md`

**状态**：
- 规划文档已落盘，当前主线进入“基于规划继续脑暴或进入第一版实现设计”阶段。
...
**用户需求**：
指出 `npc规划001.md` 放错到“导航系统重构”，要求将相关文档迁到 `.kiro/specs/NPC/`。

**完成任务**：
1. 确认 `.kiro/specs/NPC/` 已存在且为空，适合作为 NPC 的正确工作区。
2. 将 `npc规划001.md` 的主内容迁入 `.kiro/specs/NPC/npc规划001.md`。
3. 将当前文件位置的 `npc规划001.md` 改为迁移说明，停止在导航工作区继续承接 NPC 规划正文。

**状态**：
- NPC 自动移动规划的活跃维护位置已切换到 `.kiro/specs/NPC/`。
