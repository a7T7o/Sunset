...
## 2026-03-16：验收反馈触发 1.0.2纠正001，当前主线转入规则纠偏与只读取证
用户在 `2026.03.16` 这轮验收后明确指出：此前 `1.0.1自动农具进阶中断` 的实现虽然修到了一部分现象，但并没有真正理解项目当前 UI/背包/Toolbar/手持/队列/导航/预览/种植显示的实际交互规则，尤其“交换限制”不能再只按自动农具队列建模。因此农田主线已继续在 `D:\Unity\Unity_learning\Sunset\.kiro\specs\农田系统\2026.03.16\1.0.2纠正001\` 下新增一条纠偏子线，并且本轮用户要求非常明确：先全盘分析、对证、出文档，不改业务代码，等审核后再实现。当前这条新子线的只读工作已完成首版闭环：已复核实时现场为 `D:\Unity\Unity_learning\Sunset`、`codex/npc-main-recover-001 @ 8aed637f`；已回读 `GameInputManager`、`PlayerInteraction`、`InventoryInteractionManager`、`InventorySlotInteraction`、`HotbarSelectionService`、`ToolbarSlotUI`、`FarmToolPreview`、`PlacementManager`、`PlacementPreview`、`DynamicSortingOrder`、`CropController`；已通过 Unity MCP 确认活动场景仍为 `Primary` 且 Console 无新的 farm 红编译 / warning。新的稳定结论是：当前问题核心不是单点 bug，而是三条基础建模缺口叠加，即“当前正在使用中的手持内容”没有统一语义、UI 打开只被实现为轻量暂停而不是世界冻结、农田预览/队列/执行预览的所有权只按 `cellPos` 追踪过于脆弱；至于“透明幽灵作物”，本轮已收窄到预览残留 / 排序宿主错误 / 多 renderer 叠层这几条高概率嫌疑链，但尚未 live 复现，不能写成已确认根因。农田主线当前恢复点因此更新为：等待用户审核 `1.0.2纠正001` 文档口径，审核通过后再进入新一轮实现，而不是继续沿旧的 `1.0.1` 局部补丁方式推进。

## 2026-03-17：农田主线当前停在 1.0.2 任务分支提交前，尚未进入用户验收
农田主线本轮已按照用户后续授权，继续从 `1.0.2纠正001` 的只读文档阶段推进到真实实现阶段。当前实现分支为 `codex/farm-1.0.2-correct001`，当前 `HEAD` 为 `18f3a9e1`。代码层已完成三类收口：第一类是 UI/背包/Toolbar 交互保护，使用“受保护手持槽位”语义统一当前活跃手持物在背包交换、拖拽、排序、快速装备中的拒绝反馈；第二类是导航与预览冻结，UI 打开时农田导航和放置导航改为暂停/恢复，右键导航被纳入移动中断语义，`FarmToolPreview` 的队列/执行键改为 `layerIndex + cellPos`，缩小跨层残留与透明残影窗口；第三类是预览显式清理，在 `PlacementPreview.Hide()` 中同步清理 sprite、颜色、偏移与锁定位置，补强种植链显示收尾。验证层已完成 Unity MCP Console 基线和 Unity Roslyn 独立编译双闭环：活动场景仍为 `Primary`，清空 Console 后重新请求编译，未读到新的 farm `error / warning`；`Assembly-CSharp.rsp` 独立编译 `0 error / 0 warning`。当前必须写实地区分：这说明农田 `1.0.2` 已达到“可白名单提交”的本地基线，但尚未得到用户真实 PlayMode 场景验收，也尚未执行本轮任务分支的白名单 Git 收尾。因此农田主线当前唯一恢复点是：完成记忆同步后执行 `codex/farm-1.0.2-correct001` 的白名单 checkpoint，再交给用户在 Unity 现场验收。
