...
inclusion: manual
priority: P2
keywords: [维护, 清理, 重构, Steering维护, 规则管理]
...
1. 更新 `README.md` 索引
2. 更新 `smart-assistant.kiro.hook` 的领域映射（添加对应关键词→文件映射）
3. 设置正确的 front-matter（inclusion 字段）
4. 在 memory.md 记录新增原因

### 🔴 文件变更同步检查清单

每次新增/重命名/删除 steering 规则文件时，必须同步更新：
- [ ] `smart-assistant.kiro.hook` 的领域映射
- [ ] `README.md` 的文件索引表和目录结构
- [ ] `maintenance-guidelines.md` 的关键词章节（如适用）
...
2. **智能加载功能**（2026-02-11 升级）：AI 分析用户输入中的关键词后，自动用 `Read` 加载匹配的规则文件
   - 详细的关键词映射表见 `rules.md` 的"智能规则加载"章节
   - 无需用户手动输入 `#rulename`
   - 静默加载，不中断对话

3. **手动加载仍然可用**：用户仍可通过 `#rulename` 手动加载规则

### 加载行为

- ✅ AI 检测到关键词后自动加载对应规则文件
- ✅ 静默完成，不需要用户确认
- ✅ 用户也可以手动 `#rulename` 加载
- ❌ 不会加载与用户输入无关的规则
...
| 日期 | 模式 | 说明 |
|------|------|------|
| 2026-01-09 | 方案 A（手动加载 + 主动提醒） | 初始实施 |
| 2026-02-11 | 智能加载 | AI 自动分析关键词并加载，用户无感 |
