...
- `Write` 首次创建文件：建议不超过 150 行
- `Edit` 分块追加/修改：每次不超过 150 行
- 超过 200 行的内容必须分段写入
- 不要在表格或代码块中间断开

### 🔴 写入防护规则（防死循环）
- `Edit`/`Write` 调用前先在脑中确认内容非空
- 连续失败 2 次 → 立即停止重试：先 `Read` 当前文件 → 重新组织内容 → 再用 `Edit`/`Write` 落盘
- 禁止对同一文件连续调用 3 次以上失败的写入操作
- 失败后的恢复策略：`Read` 当前文件 → 确认断点 → `Edit` 精准替换/分块追加 或（仅在确认目标文件应被整体重建且用户明确要求时）使用 `Write`
