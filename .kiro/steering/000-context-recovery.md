...
- **必须重新 Read**。不允许说"我已经读过了"或"根据之前的分析"
...
- 表现：快照记录"文件 X 已修改为 Y"，但实际 Read 发现文件内容不是 Y（可能被后续对话覆盖、用户手动修改、或其他会话改动）
